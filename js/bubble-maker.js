var BubbleMaker = (function() {
  ...
  function setBubbles(list) {
    return Storage.set('bubbleList', list || []);
  }
  ...
  /* 微信风格 CSS 作用域化：把类选择器改写为带气泡唯一前缀的形式，
   * 使多个导入气泡的 .message / .message-sent / .message-received 互不串扰。
   * 先替换长选择器（.message-sent / .message-received），再替换 .message 并用负向断言
   * \.message(?![-\w]) 避免误伤 .message-sent 等子串。仅注入时动态改写，不改存储的 cssCode。 */
  function scopeWechatCss(cfg) {
    var prefix = '.bub-css-' + cfg.id;
    var code = cfg.cssCode;
    code = code.replace(/\.message-sent/g, prefix + '.message-sent');
    code = code.replace(/\.message-received/g, prefix + '.message-received');
    code = code.replace(/\.message(?![-\w])/g, prefix + '.message');
    return code;
  }
  function ensureCssBubbleStyle(cfg, force) {
    if (!cfg || cfg.source !== 'css' || !cfg.cssCode) return;
    if (!force && _injected['css_' + cfg.id]) return;
    var css = (cfg.wechat ? scopeWechatCss(cfg) : cfg.cssCode);
    styleEl().textContent += '\n' + css;
  ...
  function buildBubbleExt(msg, isSelf) {
    ...
    if (cfg.source === 'css') {
      ...
      if (cfg.wechat) {
        // 微信风格：按消息方向追加类名，并带气泡唯一前缀，配合作用域化 CSS 避免串扰
        return { extraCls: ' bub-css-' + cfg.id + ' css-bubble message message-' + (isSelf ? 'sent' : 'received'), deco: '', ears: '' };
      }
      // 非微信风格 CSS 气泡同样标记 css-bubble，用于禁用默认气泡小三角，仅显示代码本身样式
      return { extraCls: ' bub-css-' + cfg.id + ' css-bubble ' + (cfg.cssClass || ''), deco: '', ears: '' };
    }
  ...
  function renderBubbleShop() {
    var grid = el('bs-grid-all') || el('bs-maker-grid');
    if (!grid) return;
    var list = getBubbles();
    renderCardGrid(grid, list);
  }

  function miniBubbleHtml(rawCfg) {
    if (rawCfg.source === 'css') {
      if (rawCfg.wechat) {
        // 微信风格：对话式双气泡预览（上=我发送、下=对方接收）
        return '<div class="bs-mini-dialog">'
          + '<div class="bs-mini-row self"><div class="bs-mini-bubble bub-css-' + rawCfg.id + ' message message-sent">你好呀~</div></div>'
          + '<div class="bs-mini-row other"><div class="bs-mini-bubble bub-css-' + rawCfg.id + ' message message-received">你好呀~</div></div>'
          + '</div>';
      }
  ...
  function renderCardGrid(grid, list) {
    grid.innerHTML = '';
    if (!list.length) {
      grid.innerHTML = '<div class="bs-empty">还没有气泡，可点击「+ 制作新气泡」或在上方粘贴 CSS 代码创建</div>';
      return;
    }
    for (var i = 0; i < list.length; i++) {
      (function(cfg) {
        ...
        var isCss = cfg.source === 'css';
        var middleBtn = document.createElement('button');
        middleBtn.className = 'bs-card-btn';
        middleBtn.textContent = isCss ? '改名' : '编辑';
        middleBtn.addEventListener('click', isCss
          ? function() { openRename(cfg); }
  ...
  function saveRename() {
    ...
    var list = getBubbles();
    for (var i = 0; i < list.length; i++) {
      if (list[i].id === _renameTarget.id) {
        list[i].name = name;
        break;
      }
    }
    setBubbles(list);
    el('bs-rename-overlay').style.display = 'none';
    _renameTarget = null;
    renderBubbleShop();
    toast('气泡名称已修改');
  }
  ...
  function bindShopEvents() {
    ...
    bind('bs-rename-cancel', 'click', function() {
      el('bs-rename-overlay').style.display = 'none';
      _renameTarget = null;
    });
    bind('bs-rename-save', 'click', saveRename);
    bind('bs-rename-input', 'keydown', function(e) {
      if (e.key === 'Enter') saveRename();
    });
    bind('bs-rename-overlay', 'click', function(e) {
      if (e.target && e.target.id === 'bs-rename-overlay') {
        e.target.style.display = 'none';
        _renameTarget = null;
      }
    });
  ...
  /* 唯一命名：精确匹配 base 记为 1，正则 /^base\s+(\d+)$/ 取最大编号 N，
   * 返回 base（无同名）或 base + ' ' + (maxN + 1)。用"最大编号+1"而非"同名数量+1"，
   * 避免删掉中间编号后再次导入出现重复名。 */
  function uniqueBubbleName(base) {
    var esc = base.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    var re = new RegExp('^' + esc + '\\s+(\\d+)$');
    var maxN = 0;
    var names = getBubbles().map(function(b) { return b.name; });
    for (var i = 0; i < names.length; i++) {
      var exact = names[i] === base;
      var mm = re.exec(names[i]);
      if (exact && maxN < 1) maxN = 1;
      if (mm) {
  ...
  function saveCssBubble() {
    ...
    // 同名自动编号：精确同名计 1，带编号后缀取最大编号 N，新名 = base 或 base + ' ' + (maxN+1)
    var baseName = '气泡';
    var name = uniqueBubbleName(baseName);
    var cfg = {
    ...
    var list = getBubbles();
    list.push(cfg);
    var saved = setBubbles(list);
    if (!saved) {
      // 写入彻底失败（localStorage 与内存兜底均不可用）：回滚、保留输入框、明确提示失败
      list.pop();
      toast('保存失败：本地存储不可用，请清理聊天记录或改用本地服务访问');
      return;
    }
    // 双保险校验：确认新气泡已进入存储，避免"假成功"
    var persisted = getBubbles().some(function(b) { return b.id === cfg.id; });
    if (!persisted) {
      list.pop();
      toast('保存失败：数据未持久化，请重试');
      return;
    }
    ensureCssBubbleStyle(cfg);
    el('bs-css-code').value = '';
    renderBubbleShop();
    toast('气泡已保存' + (wechat ? '（我/对方自动区分方向）' : ''));
  }
