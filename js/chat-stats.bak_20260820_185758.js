...
function showChatBgPicker() {
  ...
  var colors = [
    { name: '默认', value: 'default', hex: '#F8F4F8' },
...
function applyChatBg(value) {
  ...
  var names = { 'default': '默认', '#FFE4E1': '暖粉', '#E3F2FD': '浅蓝', '#E8F5E9': '淡绿', '#FFF8E1': '奶油', '#F3E5F5': '薰衣草', '#1a1a2e': '深夜', '#1b2a1b': '墨绿' };
...
function chatHandleImage(fileInput) {
  ...
  var reader = new FileReader();
  reader.onload = function(e) {
    _compressAvatarImage(e.target.result, 800, function(compressed) {
...
function showRedPacketAction(msgId) {
  ...
  var claimBtn = canClaim
    ? '<div class="rp-action-btn primary" onclick="claimRedPacket(' + msgId + ')">领取</div>'
  ...
  var html = '<div class="rp-action-overlay" id="rp-action-overlay" onclick="closeRedPacketAction()">'
    ...
    + claimBtn
    + '<div class="rp-action-btn ghost" onclick="returnRedPacket(' + msgId + ')">退回</div>'
    + '</div>'
    + '</div></div>';
