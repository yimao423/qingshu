...
(function () {
  ...
  var BG_IMAGE_LS_LIMIT = 1024 * 1024;     // 背景图 dataURL 允许直接存 localStorage 的长度上限
  ...
  function backfillMetaFromIDB(cb) {
    ...
    var localHas = false;
    try { localHas = localStorage.getItem(META_KEY) !== null; } catch (e) { localHas = false; }
    if (localHas) return; // localStorage 已有数据，无需回填
  ...
  var BookshelfApp = {
    ...
    _renderReader: function (container) {
      ...
      var html =
        '<div class="reader-shell menu-shown" data-theme="' + shellThemeId + '"' + shellBgStyle + '">' +
          ...
          '</div>' +
          /* 分页视口（columns 分栏，左右翻页） */
          '<div class="reader-viewport" id="reader-viewport">' +
            '<div class="reader-pages" id="reader-pages" style="font-size:' + settings.fontSize + 'px;line-height:' + settings.lineHeight +
              ';text-align:' + align.id + ';' + (settings.indent ? 'text-indent:2em;' : '') +
              (isCustomBgImage ? ';background:rgba(255,255,255,0.85);border-radius:14px;padding:22px 24px;box-shadow:0 6px 24px rgba(0,0,0,0.10);' : '') + '">' +
              bodyHtml +
            '</div>' +
          '</div>' +
          /* 页码指示 */
      ...
      var paginate = function () {
        ...
        var contentW = Math.max(120, vw - 32); // 减去视口左右 padding 16*2
        pages.style.columnWidth = contentW + 'px';
        pages.style.columnGap = self._gap + 'px';
        var total = Math.max(1, Math.round(pages.scrollWidth / (contentW + self._gap)));
        var ratio = self._totalPages > 1 ? self._pageIdx / (self._totalPages - 1) : 0;
        self._totalPages = total;
        self._pageWidth = contentW + self._gap;
        if (self._pageIdx >= total) self._pageIdx = total - 1;
        // 保留阅读比例（字号/行距变化后位置不跳变）
    ...
    _applyBgImage: function (dataUrl) {
      ...
      var pages = document.querySelector('.reader-pages');
      if (pages) {
        pages.style.background = 'rgba(255,255,255,0.85)';
        pages.style.borderRadius = '14px';
        pages.style.padding = '22px 24px';
        pages.style.boxShadow = '0 6px 24px rgba(0,0,0,0.10)';
      }
      var dots = document.querySelectorAll('.reader-theme-dot');
