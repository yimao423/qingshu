...
function updateCallTimer() {
  ...
  if (el) {
    var m = Math.floor(_callSeconds / 60);
    var s = _callSeconds % 60;
    el.textContent = (m < 10 ? '0' + m : m) + ':' + (s < 10 ? '0' + s : s);
  }
  var miniTimer = document.getElementById('call-mini-timer');
  if (miniTimer) {
    var mm = Math.floor(_callSeconds / 60);
    var ss = _callSeconds % 60;
    miniTimer.textContent = (mm < 10 ? '0' + mm : mm) + ':' + (ss < 10 ? '0' + ss : ss);
  }
}
...
function hangupCall() {
  ...
  if (_callConnected && duration > 0) {
    var mm = Math.floor(duration / 60), ss = duration % 60;
...
function _triggerIncomingCall(kind) {
  ...
  ['chat-menu-overlay', 'redpacket-claim-overlay', 'redpacket-overlay', 'redpacket-list-overlay',
   'rp-action-overlay', 'rp-detail-overlay', 'photo-viewer-overlay', 'chat-bg-overlay',
  ...
  if (!records || records.length === 0) {
    ...
    records.forEach(function(rec) {
      ...
      if (rec.status === 'connected') {
        var mm = Math.floor((rec.duration || 0) / 60), ss = (rec.duration || 0) % 60;
        statusText = dirPrefix + ' · 通话时长 ' + (mm < 10 ? '0' + mm : mm) + ':' + (ss < 10 ? '0' + ss : ss);
