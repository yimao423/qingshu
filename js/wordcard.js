...
function renderWordCards() {
  ...
  var p = document.getElementById('card-grid-group');
  if (p) { renderWordCardMainGroup(); }
}
          ...
          // 格式4：「880条整理分类版本」{customReplies, customReplyGroups, ...}
            ...
            groupNames.forEach(function(gname) {
              ...
              items.forEach(function(text) {
                if (text && typeof text === 'string') {
                  cards.push({ text: text, source: '导入', category: gname });
                }
              });
            });
            ...
            if (Array.isArray(data.customReplies)) {
              ...
              groupNames.forEach(function(gname) {
                var items = groupMap[gname];
                items.forEach(function(text) {
                  if (typeof text === 'string') covered[text] = true;
                });
              });
              data.customReplies.forEach(function(text) {
                if (text && typeof text === 'string' && !covered[text]) {
                  cards.push({ text: text, source: '导入', category: '' });
                }
              });
            }
          } else {
...
function renderWordCardMain() {
  ...
  for (var i = 0; i < groups.length; i++) {
    ...
    html += '<div class="group-item-wrapper">'
          + '<div class="discover-item" onclick="openWordCardGroup(\'' + escapeHtml(g.name) + '\')">'
          ...
          + '<button onclick="event.stopPropagation();editWordCardGroupName(\'' + escapeHtml(g.name) + '\')" title="编辑分组名"><i class="fas fa-pen"></i></button>'
          + '<button class="danger" onclick="event.stopPropagation();deleteWordCardGroupItem(\'' + escapeHtml(g.name) + '\')" title="删除分组"><i class="fas fa-trash-alt"></i></button>'
          + '</div></div>';
  ...
  for (var i = 0; i < groupNames.length; i++) {
    ...
    cardsInGroup.forEach(function(c) {
      html += '<div class="search-card-item">'
            ...
            + '<button onclick="editSearchCard(\'' + escapeHtml(c.id) + '\')" title="编辑"><i class="fas fa-pen"></i></button>'
            + '<button class="danger" onclick="deleteSearchCardItem(\'' + escapeHtml(c.id) + '\')" title="删除"><i class="fas fa-trash-alt"></i></button>'
            + '</div>'
            + '</div>';
    });
    html += '</div>';
...
function _favoriteItemHtml(fav) {
  var contentHtml = '';
  if (fav.category === 'decision' && fav.decisionData) {
    ...
    contentHtml = '<img src="' + fav.stickerData + '" class="favorite-img" alt="' + Core.escapeHtml(fav.label) + '">';
  } else if (fav.imageData) {
    contentHtml = '<img src="' + fav.imageData + '" class="favorite-img" alt="图片">';
  } else {
  ...
  return '<div class="favorite-item" data-fav-id="' + Core.escapeHtml(fav.id) + '">'
    ...
    + '<button class="favorite-item-remove" onclick="removeFavorite(\'' + Core.escapeHtml(fav.id) + '\')" title="取消收藏"><i class="fas fa-star"></i></button>'
    + '</div>'
    + '</div>';
}
...
function renderCardList(cards) {
  ...
  cards.forEach(function(card) {
    ...
    var clickHandler = selMode
      ? 'toggleCardSelection(\'' + escapeHtml(card.id) + '\')'
    ...
    html += '<div class="card-list-item' + selectedClass + blockedClass + '" data-card-id="' + escapeHtml(card.id) + '"'
          ...
          + '<button onclick="editWordCard(\'' + escapeHtml(card.id) + '\')" title="编辑"><i class="fas fa-pen"></i></button>'
          + '<button class="danger" onclick="deleteWordCardItem(\'' + escapeHtml(card.id) + '\')" title="删除"><i class="fas fa-trash-alt"></i></button>'
          + '</div>'
          + '</div>';
  });
  return html;
...
function renderWordCardKaomoji() {
  ...
  for (var i = 0; i < cats.length; i++) {
    ...
    html += '<div class="group-item-wrapper">'
          + '<div class="discover-item" onclick="openKaomojiGroup(\'' + escapeHtml(g) + '\')">'
...
function renderWordCardKaomojiGroup() {
  ...
  kaomojis.forEach(function(k) {
    ...
    html += '<div class="card-list-item" data-kaomoji-id="' + escapeHtml(kid) + '">'
          + '<div class="card-list-item-body" onclick="editKaomojiItem(\'' + escapeHtml(kid) + '\')">'
          + '<div class="card-list-item-text">' + escapeHtml(k.text) + '</div>'
          + '</div>'
          + '<div class="card-list-item-actions" onclick="event.stopPropagation()">'
          + '<button onclick="editKaomojiItem(\'' + escapeHtml(kid) + '\')" title="编辑"><i class="fas fa-pen"></i></button>'
          + '<button class="danger" onclick="deleteKaomojiItem(\'' + escapeHtml(kid) + '\')" title="删除"><i class="fas fa-trash-alt"></i></button>'
          + '</div>'
          + '</div>';
  });
  container.innerHTML = html;
...
function editKaomojiItem(id) {
  ...
  body.innerHTML = '<div style="display:flex;flex-direction:column;gap:8px;width:100%">'
    ...
    + '<button class="toolbar-btn" onclick="saveKaomojiInline(\'' + escapeHtml(id) + '\')" style="font-size:0.75rem;padding:4px 12px"><i class="fas fa-check"></i> 保存</button>'
