...
function renderMoments() {
  ...
  const moments = [
    { name: '龙', avatar: '龙', color: '#C8B8E0', text: '今天在疯狗龙的情书发现了一句很美的话：心有猛虎，细嗅蔷薇。', time: Date.now() - 3600000, imgs: ['🌸', '🐯', '🌹'] },
    { name: '小明', avatar: '明', color: '#F0A0A0', text: '分享一张今天拍的照片！', time: Date.now() - 7200000, imgs: ['📷', '🌅', '☁️'] },
    { name: '小红', avatar: '红', color: '#F0C878', text: '新学了一个颜文字：(◕‿◕) 好可爱！', time: Date.now() - 18000000, imgs: [] },
    { name: '小刚', avatar: '刚', color: '#A0D8C8', text: '周末去哪里玩呢？求推荐～', time: Date.now() - 86400000, imgs: [] }
  ];
  ...
  moments.forEach(m => {
    html += `
      <div class="moment-item">
        <div class="moment-avatar" style="background:${m.color}">${m.avatar}</div>
        <div class="moment-body">
          <div class="moment-name">${m.name}</div>
          <div class="moment-text">${m.text}</div>
          ${m.imgs.length > 0 ? `<div class="moment-images">${m.imgs.map(i => `<div class="moment-img">${i}</div>`).join('')}</div>` : ''}
          <div class="moment-meta">
            <span>${Core.formatTime(m.time)}</span>
            <div class="moment-actions">
              <span><i class="far fa-heart"></i> 赞</span>
              <span><i class="far fa-comment"></i> 评论</span>
            </div>
          </div>
        </div>
      </div>
...
function renderMailbox() {
  ...
  const letters = [
    { title: '给未来的自己', time: '2026-08-04', status: '待寄出', icon: '✉️' },
    { title: '致亲爱的你', time: '2026-07-28', status: '已寄出', icon: '💌' },
    { title: '一年的约定', time: '2026-07-15', status: '已收到', icon: '📬' },
    { title: '晚安信', time: '2026-07-01', status: '已寄出', icon: '🌙' }
  ];
  ...
  letters.forEach(l => {
    html += `
      <div class="mail-item">
        <div class="mail-stamp">${l.icon}</div>
        <div class="mail-info">
          <div class="mail-title">${l.title}</div>
          <div class="mail-time">${l.time}</div>
        </div>
        <span class="mail-status">${l.status}</span>
      </div>
    `;
  });
  container.innerHTML = html;
