...
let _albumPhotoCache = {};   // 当前图集内照片 id → dataURL（避免重复读 IndexedDB）

/* 读取照片 dataURL：优先内嵌 data（旧数据兼容），否则读 IndexedDB（写入未完成时短暂重试） */
function _loadPhotoData(photo) {
  if (!photo) return Promise.resolve(null);
  if (photo.data) return Promise.resolve(photo.data);
  return AlbumPhotoDB.get(photo.id).then(function(r) {
    if (r && r.data) return r.data;
    // IndexedDB 写入可能尚未完成，300ms 后重试一次
    return new Promise(function(resolve) {
      setTimeout(function() {
        AlbumPhotoDB.get(photo.id).then(function(r2) {
          resolve(r2 && r2.data ? r2.data : null);
...
/* 相册图片压缩（全局存储架构升级）：
   canvas 重压缩，最长边限制 1920px、JPEG 质量 0.85，保证清晰度同时显著减体积；
   GIF 动图保持原样（避免丢动画）；压缩后更大则回退原图 */
function compressAlbumImage(data) {
  return new Promise(function(resolve) {
    if (!data || data.indexOf('data:') !== 0) { resolve(data); return; }
    if (data.indexOf('data:image/gif') === 0) { resolve(data); return; }
    var img = new Image();
    img.onload = function() {
      try {
        var MAX_SIDE = 1920;
        var scale = Math.min(1, MAX_SIDE / Math.max(img.width, img.height));
        if (scale >= 1 && data.length < 200 * 1024) { resolve(data); return; } // 小图且已达标：直接保留
        ...
        // PNG 含透明通道时转 JPEG 会变黑底，白底填充兼容
        var isPng = data.indexOf('data:image/png') === 0;
        if (isPng) {
...
function renderAlbum() {
  const container = document.getElementById('album-album-grid');
  if (!container) return;
  const albums = Storage.getAlbums();
  if (!albums.length) {
    container.innerHTML = '<div class="empty-state"><i class="fas fa-images"></i><p>还没有图集，点击右上角新建一个吧</p></div>';
    return;
  }
  let html = '';
  albums.forEach(function(a) {
    const count = (a.photos || []).length;
    html += '<div class="album-card" onclick="openAlbum(\'' + a.id + '\')">'
      + '<div class="album-card-cover" id="album-cover-' + a.id + '">'
      + '<div class="album-card-cover-empty"><i class="fas fa-images"></i></div>'
      + '</div>'
      + '<div class="album-card-name">' + Core.escapeHtml(a.name) + '</div>'
      + '<div class="album-card-count">' + count + ' 张照片</div>'
      + '<button class="album-card-del" onclick="event.stopPropagation();deleteAlbum(\'' + a.id + '\')" title="删除图集"><i class="fas fa-trash-alt"></i></button>'
      + '</div>';
  });
  ...
  albums.forEach(function(a) {
    ...
    if (!cover) return;
    _loadPhotoData(cover).then(function(data) {
      if (!data) return;
...
function openAlbum(id) {
  const albums = Storage.getAlbums();
  const album = albums.find(function(a) { return a.id === id; });
  if (!album) { Core.toast('图集不存在'); return; }
  _currentAlbumId = id;
  Navigation.navigateTo('album-detail');
}
...
function renderAlbumDetail() {
  ...
  const albums = Storage.getAlbums();
  const album = albums.find(function(a) { return a.id === _currentAlbumId; }) || albums[0];
  if (!album) { container.innerHTML = ''; return; }
  _currentAlbumId = album.id;
  titleEl.textContent = album.name;
  _albumPhotoCache = {};
  const photos = album.photos || [];
  if (!photos.length) {
    container.innerHTML = '<div class="empty-state"><i class="fas fa-images"></i><p>图集还是空的，点击下方按钮上传照片</p></div>';
    return;
  }
  let html = '';
  photos.forEach(function(p, i) {
    html += '<div class="album-photo-item" onclick="viewAlbumPhoto(\'' + p.id + '\')">'
      + '<img id="aph-' + p.id + '" src="" alt="照片 ' + (i + 1) + '" style="opacity:0">'
      + '<button class="album-photo-del" onclick="event.stopPropagation();deleteAlbumPhoto(\'' + p.id + '\')"><i class="fas fa-times"></i></button>'
      + '</div>';
  });
  container.innerHTML = html;
  // 异步加载照片
  photos.forEach(function(p) {
    _loadPhotoData(p).then(function(data) {
      if (!data) return;
      _albumPhotoCache[p.id] = data;
      const img = document.getElementById('aph-' + p.id);
      if (img) { img.src = data; img.style.opacity = '1'; }
    });
  });
}

/* 上传照片到当前图集（支持多选；完成后基于最新 albums 合并，防过期快照覆盖） */
function addAlbumPhoto() {
  if (!_currentAlbumId) { Core.toast('请先选择图集'); return; }
  ...
  input.onchange = function(e) {
    ...
    const batch = [];   // 本批次照片 {id, time, data, idbFailed}
    const idbTasks = [];   // {id, done, ok, promise}：done/ok 同步可读，避免 .then 异步时序竞态
    let pending = files.length;

    function commitBatch() {
      // 等 IndexedDB 原图全部落库后再合并元数据并渲染，避免读到未完成的写入
      Promise.all(idbTasks.map(function(t) {
        return t.promise.catch(function() {});
      })).then(function() {
        // 同步读取 IDB 写入结果（Promise.all 已等待，task.ok 此时必然已确定）
        var idbFailedIds = {};
        var idbFailedCount = 0;
        idbTasks.forEach(function(t) {
          if (!t.ok) { idbFailedIds[t.id] = true; idbFailedCount++; }
        });
        // 基于最新 albums 合并，避免上传期间其他改动被覆盖
        const albums = Storage.getAlbums();
        const album = albums.find(function(a) { return a.id === _currentAlbumId; });
        if (!album) { Core.toast('图集不存在'); return; }
        album.photos = album.photos || [];
        const have = {};
        album.photos.forEach(function(p) { have[p.id] = true; });
        batch.forEach(function(item) {
          if (!have[item.id]) {
            album.photos.push({
              id: item.id,
              time: item.time,
              data: idbFailedIds[item.id] ? item.data : undefined
            });
          }
        });
        // 成功与否只依据 IndexedDB 原图写入结果：全部落库即成功；
        // localStorage 镜像/元数据写入失败不影响提示（IDB 才是权威存储）
        Storage.setAlbums(albums);
        renderAlbumDetail();
        renderAlbum();
        if (idbFailedCount === 0) {
          Core.toast('已上传 ' + batch.length + ' 张照片');
        } else if (!window.indexedDB || !window.AlbumPhotoDB) {
          // IndexedDB 完全不可用（file:// 被禁用/隐私模式）：临时保存，引导使用本地服务
          Core.toast('照片已临时保存，但浏览器不支持永久存储，建议在网站目录执行 python3 -m http.server 后访问');
        } else {
          Core.toast('有 ' + idbFailedCount + ' 张照片保存失败，请检查浏览器存储权限后重试');
        }
      });
    }
    ...
    files.forEach(function(file) {
      ...
      const item = { id: id, time: Date.now(), data: null, idbFailed: false };
      batch.push(item);
      const task = { id: id, done: false, ok: false, promise: null };
      idbTasks.push(task);
      const reader = new FileReader();
      reader.onload = function(ev) {
        item.data = ev.target.result;
        // 全局存储架构升级：canvas 压缩（保清晰度、限 1920px、JPEG 0.85），减小 IndexedDB 占用
        compressAlbumImage(ev.target.result).then(function(compressed) {
          item.data = compressed;
          // 原图写入 IndexedDB（瘦身元数据后唯一可靠原图来源）；done/ok 在 promise 链上同步记录
          task.promise = AlbumPhotoDB.set({ id: id, data: compressed }).then(function() {
            task.done = true; task.ok = true;
          }, function() {
            task.done = true; task.ok = false;
          });
          pending--;
          if (pending <= 0) commitBatch();
        });
      };
...
/* 查看大图 */
function viewAlbumPhoto(id) {
  const data = _albumPhotoCache[id];
  if (!data) return;
...
function deleteAlbumPhoto(id) {
  Core.confirm('删除照片', '确定删除这张照片吗？', function() {
    const albums = Storage.getAlbums();
    const album = albums.find(function(a) { return a.id === _currentAlbumId; });
    if (album && album.photos) {
      album.photos = album.photos.filter(function(p) { return p.id !== id; });
      Storage.setAlbums(albums);
    }
    AlbumPhotoDB.del(id).catch(function() {});
    delete _albumPhotoCache[id];
    renderAlbumDetail();
    renderAlbum();
    Core.toast('照片已删除');
  });
...
function deleteAlbum(id) {
  Core.confirm('删除图集', '确定删除这个图集及其中的照片吗？', function() {
    const albums = Storage.getAlbums();
    const album = albums.find(function(a) { return a.id === id; });
    if (!album) { return; }
    (album.photos || []).forEach(function(p) {
      if (p.id) AlbumPhotoDB.del(p.id).catch(function() {});
    });
...
function openCreateAlbumModal() {
  const overlay = document.getElementById('album-create-overlay');
  const nameInput = document.getElementById('album-create-name');
...
function confirmCreateAlbum() {
  ...
  if (!name) { Core.toast('请输入图集名称'); return; }
  const albums = Storage.getAlbums();
  const id = 'album_' + Date.now();
  albums.unshift({ id: id, name: name, createdAt: Date.now(), photos: [] });
  const ok = Storage.setAlbums(albums);
  closeCreateAlbumModal();
  renderAlbum();
  // 创建后直接进入图集，引导上传照片
  _currentAlbumId = id;
  Navigation.navigateTo('album-detail');
  if (ok) Core.toast('图集已创建');
  else Core.toast('图集创建失败：浏览器存储不可用，请检查存储权限或改用本地服务访问');
}
...
const ShopApp = {
  products: [
    { id: 'p1', name: '情书水晶球', price: 128, icon: '🔮', desc: '精致水晶球摆件' },
...
const SurveyApp = {
  surveys: [
    {
      ...
      desc: '帮助我们做得更好～',
      questions: [
        { id: 'q1', type: 'single', title: '你对疯狗龙的情书的整体满意度如何？', options: ['非常满意', '满意', '一般', '不满意', '非常不满意'] },
        { id: 'q2', type: 'multiple', title: '你最喜欢哪些功能？（多选）', options: ['聊天', '字卡', '相册', '树洞', '音乐', '小说', '记事本', '商城'] },
        { id: 'q3', type: 'single', title: '你希望增加什么新功能？', options: ['日记', '记账', '日程管理', '游戏', '其他'] }
      ]
    }
  ],
  currentSurvey: null,
  ...
  renderList() {
    ...
    this.surveys.forEach(s => {
      html += `
        <div class="survey-card glass-section" onclick="SurveyApp.startSurvey('${s.id}')">
          <div class="survey-card-icon"><i class="fas fa-clipboard-list"></i></div>
          <div class="survey-card-info">
            <div class="survey-card-title">${s.title}</div>
            <div class="survey-card-desc">${s.desc} · ${s.questions.length}题</div>
          </div>
  ...
  startSurvey(id) {
    ...
    Navigation.navigateTo('survey-answer');
  },

  renderQuestions() {
    const survey = this.surveys.find(s => s.id === this.currentSurvey);
    if (!survey) return;
    const container = document.getElementById('survey-questions');
    const titleEl = document.getElementById('survey-answer-title');
    if (titleEl) titleEl.textContent = survey.title;
    if (!container) return;

    let html = '';
    survey.questions.forEach(q => {
      html += `<div class="survey-question glass-section">
        <div class="question-title">${q.title}</div>
        <div class="question-type-label">${q.type === 'single' ? '[单选]' : '[多选]'}</div>`;

      q.options.forEach(opt => {
        const checked = this.answers[q.id] && (Array.isArray(this.answers[q.id]) ? this.answers[q.id].includes(opt) : this.answers[q.id] === opt);
        html += `
        <label class="question-option">
          <input type="${q.type === 'single' ? 'radio' : 'checkbox'}" name="q-${q.id}" value="${opt}" ${checked ? 'checked' : ''} 
            onchange="SurveyApp.recordAnswer('${q.id}', '${opt}', this)">
  ...
  submitSurvey() {
    Core.toast('问卷已提交，感谢参与！');
    setTimeout(() => Navigation.goBack(), 1000);
  }
};

function renderSurvey() {
  SurveyApp.renderList();
}

function renderSurveyAnswer() {
  // Handled by startSurvey
}
