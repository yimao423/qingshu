...
const ShopApp = {
  ...
  buyForSelf(productId) {
    ...
    const msg = this._buildGiftMsg(p, { isSelf: true, note: '我在拾心商城买下了「' + p.name + '」', greeting: '自己给自己一份小确幸～' });
  ...
  askPartnerPay(productId) {
    ...
    setTimeout(() => {
      ...
      if (agree) {
        ...
        const msg = this._buildGiftMsg(p, { isSelf: false, note: '我付款啦，给你买了「' + p.name + '」', greeting: '只要是你喜欢的，我都愿意买单～' });
        this._pushGiftToChat(chatId, msg);
  ...
  giftToPartner(productId) {
    ...
    const msg = this._buildGiftMsg(p, { isSelf: true, note: '我送你的「' + p.name + '」收到没？', greeting: '想把所有好的都给你' });
    this._pushGiftToChat(chatId, msg);
    Core.toast('礼物已送出！');
  },
