...
function doodleSend() {
  ...
  compressImageData(data, 240, 0.9, false).then(function(compressed) {
...
function _doSendDoodle(side, stickerData, chatId) {
  ...
  messages.push({ id: Date.now(), type: side, text: '[涂鸦]', time: Date.now(), msgType: 'doodle', read: side !== 'self', stickerData: stickerData });
...
function _sendPatAutoReply(chatId) {
  ...
  var finalText = _buildPatText(template, 'other', isGroup);
  var messages = Storage.getMessages(chatId);
  messages.push({
    id: Date.now(),
    type: 'other',
...
/* 铃铛禁入图标（黑屋通知气泡内使用，固定粉红色） */
var BLACK_NOTICE_ICON_SVG = '<svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" width="14" height="14"><path d="M16 3.9 C11.2 3.9 8.3 7.8 8.3 12.1 V16.8 L6 21.2 C5.6 22 6.1 22.9 7 22.9 H25 C25.9 22.9 26.4 22 26 21.2 L23.7 16.8 V12.1 C23.7 7.8 20.8 3.9 16 3.9 Z" fill="#D84A6A"/><rect x="13.2" y="24.7" width="5.6" height="3.1" rx="1.55" fill="#D84A6A"/><path d="M4.2 4.2 L27.8 27.8" stroke="#D84A6A" stroke-width="2.7" stroke-linecap="round"/></svg>';
...
/* 小黑屋弹窗：选择成员打入冷宫（单聊/群聊通用；单聊对象为当前对方） */
function openBlackRoomPanel() {
  ...
  var html = '<div class="black-overlay" id="black-room-overlay" onclick="closeBlackRoomPanel()">'
    ...
    + '<label>选择打入冷宫的成员（点击切换）</label>'
    + '<div class="black-member-list" id="black-room-member-list">' + memberHtml + '</div>'
    + '<div class="black-hint">被关进小黑屋的成员，发送的消息会附带「已被打入冷宫」标签</div>'
    + '</div>'
    + '<div class="black-actions">'
    + '<button class="black-btn black-btn-cancel" onclick="closeBlackRoomPanel()">取消</button>'
    + '<button class="black-btn black-btn-primary" onclick="saveBlackRoom()">确认</button>'
    + '</div>'
    + '</div></div>';
...
function saveBlackRoom() {
  var chatId = _currentChatId();
  if (!chatId) return;
  var chips = document.querySelectorAll('#black-room-member-list .black-member-chip.active');
  var ids = [];
  chips.forEach(function(c) { ids.push(c.dataset.member); });
  setBlackRoomIds(chatId, ids);
  closeBlackRoomPanel();
  renderChatMessages(chatId);
  Core.toast(ids.length ? '已关入小黑屋 ' + ids.length + ' 人' : '小黑屋已清空');
}

/* 黑屋通知弹窗：选择成员 + 自定义/内置恋爱向通知（单聊/群聊通用） */
function openBlackNoticePanel() {
  closePlusMenu();
  var chatId = _currentChatId();
  if (!chatId) { Core.toast('请先进入聊天'); return; }
  var isGroup = isGroupChatId(chatId);
  ...
  members.forEach(function(m) {
    var av = _decisionChipAvatarHtml(m);
    memberHtml += '<div class="black-member-chip" data-member="' + m.id + '">' + av + Core.escapeHtml(m.nickname || '成员') + '</div>';
  });

  var presetHtml = '';
  BLACK_NOTICE_PRESETS.forEach(function(text) {
    presetHtml += '<div class="black-preset-item"><span class="preset-tag">♡</span>' + Core.escapeHtml(text) + '</div>';
  });

  var html = '<div class="black-overlay" id="black-notice-overlay" onclick="closeBlackNoticePanel()">'
    + '<div class="black-panel" onclick="event.stopPropagation()">'
    + '<div class="black-title"><i class="fas fa-bell-slash"></i>黑屋通知</div>'
    + '<div class="black-field">'
    + '<label>通知对象（可多选）</label>'
    + '<div class="black-member-list" id="black-notice-member-list">' + memberHtml + '</div>'
    + '</div>'
    + '<div class="black-field">'
    + '<label>自定义通知</label>'
    + '<textarea class="black-input" id="black-notice-custom" placeholder="输入想对冷宫成员说的话…" maxlength="100"></textarea>'
    + '</div>'
    + '<div class="black-field">'
    + '<label>恋爱向黑屋通知（点击填入上方输入框）</label>'
    + '<div class="black-preset-list" id="black-notice-preset-list">' + presetHtml + '</div>'
    + '</div>'
    + '<div class="black-actions">'
    + '<button class="black-btn black-btn-cancel" onclick="closeBlackNoticePanel()">取消</button>'
    + '<button class="black-btn black-btn-primary" onclick="sendBlackNotice()">发送通知</button>'
    + '</div>'
    + '</div></div>';
  ...
  var list = document.getElementById('black-notice-member-list');
  if (list) {
    list.addEventListener('click', function(e) {
      var chip = e.target.closest ? e.target.closest('.black-member-chip') : null;
      if (!chip) return;
  ...
  if (presetList) {
    presetList.addEventListener('click', function(e) {
      ...
      if (input) input.value = (item.textContent || '').replace(/^♡\s*/, '').trim();
      var items = presetList.querySelectorAll('.black-preset-item');
      items.forEach(function(x) { x.style.borderColor = ''; x.style.background = ''; });
      item.style.borderColor = 'var(--primary, #F0A8BC)';
      item.style.background = 'rgba(var(--primary-rgb, 255,150,178), 0.25)';
    });
  }
  var overlay = document.getElementById('black-notice-overlay');
  if (overlay) {
    overlay.addEventListener('touchmove', function(e) { e.preventDefault(); }, { passive: false });
  }
}

function closeBlackNoticePanel() {
  var el = document.getElementById('black-notice-overlay');
  if (el) el.remove();
}

/* 发送黑屋通知（居中气泡，粉红色系，类似撤回/通话气泡） */
/* 发布后，被通知对象会自动回复「收到通知：xxx + 我知道错了😭」 */
function sendBlackNotice() {
  var chatId = _currentChatId();
  if (!chatId) return;
  var chips = document.querySelectorAll('#black-notice-member-list .black-member-chip.active');
  if (!chips.length) { Core.toast('请选择通知对象'); return; }
  var input = document.getElementById('black-notice-custom');
  var text = (input && input.value || '').trim();
  if (!text) { Core.toast('请输入通知内容'); return; }
  ...
  chips.forEach(function(c) {
    ...
    if (isGroup) {
      ...
      for (var i = 0; i < members.length; i++) {
        if (members[i].id === c.dataset.member) { names.push(members[i].nickname || '成员'); break; }
      }
    } else {
      var partner = _getCurrentPartnerProfile();
      if (partner && partner.id === c.dataset.member) { names.push(partner.nickname || '对方'); }
    }
  });
  // 通知「全员」时：单聊取当前对方，群聊取群内所有成员作为回复对象
  if (hasAll && !isGroup) {
    var partner0 = _getCurrentPartnerProfile();
    if (partner0) { memberIds.push(partner0.id); names.push(partner0.nickname || '对方'); }
  }
  if (hasAll && isGroup) {
    var gAll = getGroupByChatId(chatId);
    var allMembers = gAll ? getGroupMembers(gAll) : [];
    allMembers.forEach(function(m) {
      if (memberIds.indexOf(m.id) === -1) { memberIds.push(m.id); names.push(m.nickname || '成员'); }
    });
  }
  ...
  closeBlackNoticePanel();
  renderChatMessages(chatId);
  App.playSound('send');

  // 被通知对象自动回复：收到通知：内容 + 我知道错了😭
  scheduleBlackNoticeReply(chatId, memberIds.slice(), text);
}

/* 模拟被通知方回复「收到通知：xxx + 我知道错了😭」 */
function scheduleBlackNoticeReply(chatId, memberIds, noticeText) {
  ...
  setTimeout(function() {
    if (Storage.getTypingIndicator && Storage.getTypingIndicator()) {
      ...
      setTimeout(function() { _doBlackNoticeReply(chatId, replyTargets, noticeText, isGroup); }, 1400 + Math.random() * 2000);
    } else {
      _doBlackNoticeReply(chatId, replyTargets, noticeText, isGroup);
    }
  }, delay);
}

function _doBlackNoticeReply(chatId, replyTargets, noticeText, isGroup) {
  hideTypingIndicator();
  var replyText = '收到通知："' + noticeText + '"我知道错了😭';
  if (isGroup && replyTargets.length) {
    replyTargets.forEach(function(member, idx) {
      setTimeout(function() {
        var msgs = Storage.getMessages(chatId);
        msgs.push({ id: Date.now() + idx, type: 'other', fromId: member.id, text: replyText, time: Date.now(), msgType: 'text' });
  ...
  var msgs = Storage.getMessages(chatId);
  msgs.push({ id: Date.now(), type: 'other', text: replyText, time: Date.now(), msgType: 'text' });
  Storage.setMessages(chatId, msgs);
...
/* 打开拍一拍弹窗：mode='self' 我方发送；mode='other' 对方角色自动回复发布 */
function openPatPanel(mode, presetText) {
  ...
  presetPats.forEach(function(p) {
    ...
    presetHtml += '<div class="black-preset-item"><span class="preset-tag">♡</span>' + Core.escapeHtml('"我方"' + _a + '"对方"' + _b) + '</div>';
  });

  var isOther = mode === 'other';
  var title = isOther ? '拍一拍（对方发布）' : '拍一拍';
  ...
  var html = '<div class="black-overlay" id="pat-overlay" onclick="closePatPanel()">'
    ...
    + '<div class="black-title"><i class="fas fa-hand-point-up"></i>' + title + '</div>'
    + '<div class="black-field">'
    + '<label>（"我方"后的内容）</label>'
    + '<input class="black-input" id="pat-custom-a" placeholder="如：拍了拍" maxlength="30">'
    + '</div>'
    + '<div class="black-field">'
    + '<label>（"对方"后的内容）</label>'
    + '<input class="black-input" id="pat-custom-b" placeholder="如：想你了" maxlength="30">'
    + '</div>'
    + '<div class="black-field">'
    + '<label>恋爱向拍一拍（点击填入上方两个输入框）</label>'
    + '<div class="black-preset-list" id="pat-preset-list">' + presetHtml + '</div>'
    + '</div>'
    + '<div class="black-actions">'
  ...
  if (presetList) {
    presetList.addEventListener('click', function(e) {
      var item = e.target.closest ? e.target.closest('.black-preset-item') : null;
      if (!item) return;
      var tpl = (item.textContent || '').replace(/^♡\s*/, '').trim();
      var parts = _splitPatTemplate(tpl);
      var inputA = document.getElementById('pat-custom-a');
      var inputB = document.getElementById('pat-custom-b');
      if (inputA) inputA.value = parts.a;
      if (inputB) inputB.value = parts.b;
      var items = presetList.querySelectorAll('.black-preset-item');
      items.forEach(function(x) { x.style.borderColor = ''; x.style.background = ''; });
      item.style.borderColor = 'var(--primary, #F0A8BC)';
      item.style.background = 'rgba(var(--primary-rgb, 255,150,178), 0.25)';
...
function _buildPatText(template, mode, isGroup) {
  var selfName = '我';
  var otherName = isGroup ? (_getCurrentGroupName() || '群聊') : _getCurrentPartnerName();
...
function sendPat(mode) {
  ...
  var isGroup = isGroupChatId(chatId);
  var template = '"我方"' + a + '"对方"' + b;
  var finalText = _buildPatText(template, mode, isGroup);
...
/* ============================================================
   ...
   ============================================================ */
var PAT_AUTO_REPLY_MIX_PROBABILITY = 0.15; // 15%，低于红包(25%)/表情包(40%)混入概率
...
var PUNISH_ICON_SVG = '<svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" width="14" height="14"><path d="M7 5 L14.5 12.5" stroke="#D8404E" stroke-width="4" stroke-linecap="round"/><path d="M9 7 L12.2 10.2" stroke="#fff" stroke-width="1.6" stroke-linecap="round"/><path d="M14.5 12.5 C20 8.5 26 9 28.5 12.5 C30.5 15.5 28 18.5 24.5 18.8 C22 19 20 17.8 19.2 16" stroke="#D8404E" stroke-width="2.6" stroke-linecap="round" fill="none"/></svg>';
