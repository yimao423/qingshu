...
function openPatPanel(mode, presetText) {
  ...
  var html = '<div class="black-overlay" id="pat-overlay" onclick="closePatPanel()">'
    ...
    + '<div class="black-field pat-preset-field">'
  ...
  if (overlay) {
    // 预设区（.pat-preset-grid / .pat-preset-field）需保留原生滚动：
    // 触摸起点落在预设区内时不拦截 touchmove，PC 端滚轮同理不受影响；
    // 其余区域仍拦截，避免弹窗背后的聊天列表被连带滚动
    overlay.addEventListener('touchmove', function(e) {
      var t = e.target;
      var inPreset = !!(t && t.closest && t.closest('.pat-preset-grid'));
      if (inPreset) return;
      e.preventDefault();
    }, { passive: false });
  }
