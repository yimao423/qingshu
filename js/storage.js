...
/* ============================================================
   ...
   聊天发送链路已回归内联存储：聊天图片/表情压缩后 base64 直接写入消息对象，
   不再转 __IDB_IMG__ 引用（旧 WebView 下异步还原不可靠）。
   本库继续服务备份导出 media 抽取、导入恢复与历史 __IDB_IMG__ 引用兼容。
   ============================================================ */
...
/* ============================================================
   ChatMedia: 图片/表情「阈值判定」压缩 + 媒体工具层（对标 ZY3）
   - 阈值判定：小于阈值的小图/小表情原样保存，不做无谓压缩损耗；
   - GIF 动图：任何情况下原样返回，保证动图正常播放，绝不压成静态图；
   - 聊天消息压缩后内联存储（storeForMessage 恒内联），渲染同步显示；
   - 历史 __IDB_IMG__ 引用 / 备份导出 media 仍走 ChatImageDB 兼容还原。
   ============================================================ */
var ChatMedia = {
  ...
  /* 消息落库存储：聊天媒体统一内联（压缩后 base64 直接入库，不再引用化） */
  /* 聊天媒体统一内联存储：压缩后的 base64 直接写入消息对象，渲染同步显示，
     不再转 __IDB_IMG__ 引用（旧 WebView 下异步还原不可靠）。
     历史消息中的 __IDB_IMG__ 引用仍由 getData/resolveDomRefs 兼容还原。 */
  ...
  resolveDomRefs: function(rootEl) {
  ...
  /* 导出瘦身（单条消息）：引用 → 收集 IDB 数据到 mediaMap；内联大 base64 → 抽取为引用 + media */
  collectMsgMedia: function(msg, mediaMap, seqBox, threshold) {
    var self = this;
    var t = threshold || self.THRESHOLD;
    var jobs = [];
    function pick(obj, f, prefix) {
      var v = obj && obj[f];
      if (!v || v.indexOf('data:') !== 0 && !self.isRef(v)) return;
      if (self.isRef(v)) {
        var k = self.refKey(v);
        if (!mediaMap[k]) {
          jobs.push(ChatImageDB.get(k).then(function(data) {
            if (data) mediaMap[k] = data;
          }).catch(function() {}));
        }
        return;
      }
      if (v.length > t) {
        var key = 'exp_' + prefix + '_' + (msg && msg.id !== undefined ? msg.id : 'x') + '_' + (seqBox[0]++);
        mediaMap[key] = v;
        obj[f] = self.makeRef(key);
      }
    }
    pick(msg, 'stickerData', 'stk');
    pick(msg, 'imageData', 'img');
    if (msg) {
      pick(msg.recalledContent, 'stickerData', 'rc');
      pick(msg.recalledContent, 'imageData', 'rc');
      pick(msg.quote, 'stickerData', 'qt');
      pick(msg.quote, 'imageData', 'qt');
    }
    return Promise.all(jobs);
  },
  ...
  /* 导出瘦身（表情包）：深拷贝后把超阈值的内联 base64（含 GIF，仅搬运不压缩）抽成引用 + media。
     表情库渲染直接 <img src>，故真实数据保持内联；导出副本引用化，导入时由 resolveStickerForImport 还原。 */
  collectStickersForExport: function(stickers, mediaMap, seqBox, threshold) {
    ...
    copy.forEach(function(s) {
      ...
      if (self.isRef(s.data)) return;
      if (s.data.length > t) {
        var key = 'lib_' + (s.id !== undefined && s.id !== null ? s.id : ('s' + seqBox[0]++));
        mediaMap[key] = s.data;
        s.data = self.makeRef(key);
      }
    });
    return copy;
  },
