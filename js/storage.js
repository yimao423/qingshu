...
var AppKVDB = (function() {
  ...
  function open() {
    ...
    _opening = new Promise(function(resolve, reject) {
      ...
      // 超时兜底：file:// / 隐私模式等环境下 open 可能永不回调，若不加超时，
      // 上层 getAsync / _asyncVerifyFromIDB 的 Promise 将永久 pending（.then 永不触发），
      // 造成"IDB 权威恢复彻底失效、刷新后数据回退默认"（主题/聊天首帧回退浅蓝即此症状）。
      ...
      req.onerror = function() { clearTimeout(_timer); _opening = null; reject(req.error); };
      req.onblocked = function() { clearTimeout(_timer); _opening = null; reject(new Error('indexedDB blocked')); };
    });
    return _opening;
  }
...
var MessageDB = (function() {
  ...
  function open() {
    ...
    _opening = new Promise(function(resolve, reject) {
      var req = indexedDB.open(DB_NAME, DB_VER);
      // 超时兜底（同 AppKVDB）：避免文件/隐私模式下永久 pending，导致聊天异步恢复失效
      var _timer = setTimeout(function() {
        _opening = null;
        reject(new Error('indexedDB open timeout'));
      }, 3000);
...
const Storage = {
  ...
  // 异步权威读取：内存缓存 → localStorage 镜像 → IndexedDB 权威层，按序兜底。
  // 仅在 IDB 命中时回填内存 + 镜像（不广播事件、由调用方决定是否 apply），
  // 与 _asyncVerifyFromIDB 防重入互不冲突（本方法每 key 独立 Promise）。
  getAsync(key, defaultValue = null) {
    var self = this;
    return new Promise(function (resolve) {
      // 内存缓存命中
      if (self._memCache && key in self._memCache) {
        resolve(self._memCache[key]);
        return;
      }
      // localStorage 镜像命中
      try {
        var v = localStorage.getItem(self.PREFIX + key);
        if (v !== null) { resolve(JSON.parse(v)); return; }
      } catch (e) {}
      // IndexedDB 权威层兜底恢复
      if (window.AppKVDB && self._isKVMirrorable(key)) {
        AppKVDB.get(self.PREFIX + key).then(function (record) {
          if (record && record.value !== undefined) {
            self._memCache[key] = record.value;
            self._memCache['__ts_' + key] = String(record.updatedAt || Date.now());
            try {
              localStorage.setItem(self.PREFIX + key, JSON.stringify(record.value));
              localStorage.setItem(self.PREFIX + '__ts_' + key, String(record.updatedAt || Date.now()));
            } catch (e2) {}
  ...
  set(key, value) {
    ...
    // 聊天镜像（msg_*）：走专用容量保护写入（权威在 MessageDB，localStorage 仅提速镜像）
    if (key.indexOf('msg_') === 0) {
      this._writeMsgMirror(key.slice(4), value);
      return true;
    }
    ...
    try {
      ...
      // 配额满：自动腾挪一条最旧的聊天镜像后再试（聊天权威在 MessageDB，删镜像不丢数据），
      // 仍失败才降级"仅 IDB 持久化"——否则主题/图标等小设置将被聊天镜像塞满配额而静默丢失，
      // 刷新即回退默认（PC file:// 下 IDB 不可用会更彻底丢失）。
      if (e && (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED')) {
        if (this._evictOldestMsgMirror()) {
          try {
            localStorage.setItem(this.PREFIX + key, JSON.stringify(value));
            localStorage.setItem(this.PREFIX + '__ts_' + key, String(now));
            lsOk = true;
          } catch (e2) {
            console.warn('Storage.set localStorage 镜像失败（腾挪后仍配额不足），数据仅由 IndexedDB 持久化:', e2.message);
          }
        } else {
          console.warn('Storage.set localStorage 镜像失败（配额满且无聊天镜像可腾挪），数据将仅由 IndexedDB 持久化:', e.message);
        }
      } else {
        console.warn('Storage.set localStorage 镜像失败（隐私模式等），数据将仅由 IndexedDB 持久化:', e.message);
      }
    }
    // IndexedDB 权威持久化（跟随设备永久保存，无 5MB 配额限制）
    // 挂到全局写链上串行落盘，flush() 返回该链，供主题保存后等待实际落库（修复1：避免"保存完即刷新丢主题"）
    var self = this;
    var hasIDB = !!(window.AppKVDB && this._isKVMirrorable(key));
    if (hasIDB) {
      this._pendingIdbWrites = this._pendingIdbWrites || Promise.resolve();
      this._pendingIdbWrites = this._pendingIdbWrites.then(function () {
        return AppKVDB.put({ key: self.PREFIX + key, value: value, updatedAt: now }).catch(function () {});
      });
      return true;
    }
    return lsOk;
  ...
  // 返回 IndexedDB 写链 Promise：等待此前所有 Storage.set 的 IDB 落盘完成
  flush() {
    return this._pendingIdbWrites || Promise.resolve();
  },

  /* 配额满时的自动腾挪：删除最旧的一条聊天镜像（mirror_msg_*）。
     聊天记录权威在 MessageDB(IndexedDB)，localStorage 里的 mirror_msg_* 仅历史遗留的提速镜像，
     删除镜像不丢任何聊天数据，却能立即腾出空间让主题/图标等小配置镜像写入成功。
     因此即使 localStorage 已被历史聊天镜像塞满，下一次保存主题也会自动成功。 */
  _evictOldestMsgMirror() {
    var bestKey = null, bestTs = Infinity;
    try {
      for (var i = 0; i < localStorage.length; i++) {
        var k = localStorage.key(i);
        if (!k || k.indexOf(this.PREFIX + 'msg_') !== 0) continue;
        var ts = 0;
        try {
          ts = parseInt(localStorage.getItem(this.PREFIX + '__ts_' + k.slice(this.PREFIX.length)) || '0', 10) || 0;
        } catch (e) {}
      ...
      if (!bestKey) {
        ...
        for (var j = 0; j < localStorage.length; j++) {
          ...
          if (k2 && k2.indexOf(this.PREFIX + 'msg_') === 0) { bestKey = k2; break; }
        }
      }
      if (bestKey) {
        localStorage.removeItem(bestKey);
        var shortKey = bestKey.slice(this.PREFIX.length);
        try { localStorage.removeItem(this.PREFIX + '__ts_' + shortKey); } catch (e) {}
        // 注意：不动 _msgCache（内存消息缓存是当前渲染数据源，删它会导致打开的聊天回退读空；
        // 聊天权威在 MessageDB，localStorage 镜像删除不影响数据完整性）
        return true;
      }
    } catch (e) {}
    return false;
  ...
  /* 聊天镜像受控写入：大体积聊天不进 localStorage（权威在 MessageDB，读取侧有 IDB 异步兜底），
     配额满时腾挪重试。避免长对话/图片撑爆 5MB 配额，拖垮 theme/iconZones 等其他设置的持久化。 */
  _writeMsgMirror(chatId, messages) {
    var json;
    try { json = JSON.stringify(messages); } catch (e) { return; }
    if (!json) return;
    // 超过约 400KB 的单条聊天镜像跳过（镜像价值低、耗配额高）
    if (json.length > 400 * 1024) return;
    var full = this.PREFIX + 'msg_' + chatId;
    var tsKey = this.PREFIX + '__ts_msg_' + chatId;
    try {
      localStorage.setItem(full, json);
      try { if (!localStorage.getItem(tsKey)) localStorage.setItem(tsKey, String(Date.now())); } catch (e) {}
    } catch (e) {
      if (e && (e.name === 'QuotaExceededError' || e.name === 'NS_ERROR_DOM_QUOTA_REACHED')) {
        if (this._evictOldestMsgMirror()) {
          try { localStorage.setItem(full, json); } catch (e2) {}
        }
      }
    }
  },
  ...
  /* 最近使用的 emoji（聊天面板「最近使用」分区，最新在前，最多9个） */
  getRecentEmojis() {
  ...
  // 涂鸦混入：默认开启（保持原有「对方自动回复 10% 概率随机发涂鸦」的行为不变）
  getDoodleMixing() { return this.get('doodleMixing', true); },
  setDoodleMixing(v) { this.set('doodleMixing', v); },
  ...
  // 对方偶尔转发消息给你的概率（0~1，默认 0.05，仅当对方能正常发送时才会触发）
  getPartnerForwardProb() { return this.get('partnerForwardProb', 0.05); },
  setPartnerForwardProb(v) {
    var val = Number(v);
    if (isNaN(val)) val = 0;
    this.set('partnerForwardProb', Math.max(0, Math.min(1, val)));
  },
...
/* ============================================================
   ChatImageDB: 聊天大图/大表情 IndexedDB 存储层（轻量引用存储模式）
...
var ChatMedia = {
  ...
  /* 按场景设定压缩参数：小图不压、超阈值才压缩、按场景定宽/质量 */
  OPT: {
    image:    { maxWidth: 1200, quality: 0.8  }, // 聊天图片：原 480px 过小，提升至 1200px
