...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // 全局存储架构升级：清理 localStorage 旧数据残留（含 base64 大图的 albums/photos 键），
  // 修复"IndexedDB 已权威持久化但仍报存储空间不足"；同时检测 IndexedDB 可用性并给出引导
  setTimeout(function() {
    if (window.Storage && typeof Storage._purgeLegacyAlbumData === 'function') {
      Storage._purgeLegacyAlbumData();
    }
...
window.filterWordCards = filterWordCards;
window.clearWordCardFilter = clearWordCardFilter;
window.toggleWordcardMenu = toggleWordcardMenu;
...
window.openWriteLetter = openWriteLetter;
