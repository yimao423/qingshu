...
(function () {
  ...
  function pauseWorld() {
    ...
    try {
      ...
      // 收起可能残留的来电 / 通话浮层
      ['call-incoming-overlay', 'call-active-overlay'].forEach(function (id) {
  ...
  function entryInvite() {
    ...
    ensureFrame(function (ok) {
      if (!ok) { alert('镜界引擎加载失败，无法发起传讯'); return; }
      var cid = currentChatId();
  ...
  function showInviteForSingle(cid) {
    ...
    openOverlay(
      ...
      + '<div class="mtx-modal-desc">邀请 <b>' + esc(name) + '</b> 进入次元镜界传讯模式。<br>TA 将按镜界抽牌逻辑，用牌面图片回复你的每一个问题。</div>'
  ...
  function loadSession(id) {
    try {
      var raw = localStorage.getItem(K + 'session_' + id);
      return raw ? JSON.parse(raw) : null;
    } catch (e) { return null; }
  }
  function saveSession() {
    if (!session) return;
    try { localStorage.setItem(K + 'session_' + session.id, JSON.stringify(session)); } catch (e) { /* 空间不足忽略 */ }
  }
  function loadIndex() {
    try { return JSON.parse(localStorage.getItem(K + 'index') || '[]'); } catch (e) { return []; }
  }
  function updateIndex(extra) {
    ...
    var entry = {
      ...
      ended: session.ended, qCount: Math.max(0, session.items.filter(function (i) { return i.type === 'q'; }).length)
    };
    ...
    try { localStorage.setItem(K + 'index', JSON.stringify(idx)); } catch (e) { /* 忽略 */ }
  }
  ...
  function deleteSession(id) {
    cleanupSessionMedia(loadSession(id));
    try { localStorage.removeItem(K + 'session_' + id); } catch (e) {}
    try {
  ...
  function finishHand(cards, m, q, cb) {
    ...
    var jobs = cards.map(function (c, i) {
      if (c.image && typeof ChatMedia !== 'undefined' && ChatMedia.storeForMessage && String(c.image).indexOf('data:') === 0) {
        return ChatMedia.storeForMessage(c.image, 'mtx_' + session.id + '_' + handSeq + '_' + i).then(function (ref) { c.image = ref; return c; });
      }
  ...
  function doEndTransmit() {
    ...
    saveSession();
    updateIndex({ ended: session.ended, qCount: session.items.filter(function (i) { return i.type === 'q'; }).length, done: 1 });
    resumeWorld();
    session = null;
    questionActive = false;
    closeOverlay();
  ...
  function openEchoPage() {
    ...
    if (!box) return;
    box.innerHTML = '';
    var idx = loadIndex().filter(function (e) { return e.ended; });
    var single = [], group = [];
    idx.forEach(function (e) { (e.kind === 'group' ? group : single).push(e); });
  ...
  function renderEchoSessionList() {
    ...
    var idx = loadIndex().filter(function (e) {
      return e.ended && e.kind === echoView.kind && (e.targetName || '') === echoView.targetName;
    });
    if (!idx.length) { box.innerHTML = '<div class="mtx-empty-page">暂无该对象的传讯轮次</div>'; return; }
    var html = '';
    idx.forEach(function (e) {
      var s = loadSession(e.id);
      var qn = s ? s.items.filter(function (i) { return i.type === 'q'; }).length : 0;
      html += '<div class="mtx-echo-sess" onclick="MTXApp.openEchoSession(\'' + esc(e.id) + '\')">'
        ...
        + '<div class="mtx-echo-sess-title">第 ' + (e.created ? fmtTimeFull(e.created) : '') + ' 轮</div>'
        + '<div class="mtx-echo-sess-sub">' + qn + ' 问 · ' + (s ? s.members.length : 0) + ' 位参与者</div>'
        + '</div>'
  ...
  function renderEchoSessionPage() {
    ...
    if (!s) { play.innerHTML = '<div class="mtx-empty-page">记录已被删除</div>'; return; }
