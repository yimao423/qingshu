...
function renderTypingSymbolSelection() {
  var current = Storage.getTypingSymbol() || '❤';
...
function renderPatSymbolSelection() {
  var currentSelf = Storage.getPatSelfSymbol() || '♥';
  var currentOther = Storage.getPatOtherSymbol() || '♥';
  var map = [
...
function showTypingIndicator(name) {
  ...
  var sym = Storage.getTypingSymbol() || '❤';
...
function scheduleReadReceipt(chatId, msgId) {
  ...
  _readTimers[key] = setTimeout(function() {
    ...
    if (changed) {
      ...
      var room = document.getElementById('page-chat-room');
      if (room && room.dataset.chatId === chatId) renderChatMessages(chatId);
    }
  }, delay);
}
...
function startKeepAliveAudio() {
  ...
  try {
    ...
    var len = Math.max(1, Math.floor(sampleRate)); // 1 秒静音缓冲
    _keepAliveBuffer = _keepAliveCtx.createBuffer(1, len, sampleRate);
    var data = _keepAliveBuffer.getChannelData(0);
    for (var i = 0; i < len; i++) data[i] = 0; // 全静音
