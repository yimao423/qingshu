...
(function () {
  ...
  window.ScienceApp = {
    ...
    _all: function () {
      return this._getCustom().concat(SCIENCE_BUILTIN);
    },
    ...
    saveForm: function (editId) {
      ...
      if (editId) {
        ...
        if (!found) { toast('未找到该科普，可能已被删除'); this._closeOverlay(); this.render(); return; }
      } else {
    ...
    _detailHtml: function () {
      ...
      var editDel = a.builtin ? '' :
        '<div class="science-detail-admin">' +
          '<button class="glass-btn" onclick="ScienceApp.openEdit(\'' + a.id + '\')"><i class="fas fa-edit"></i> 编辑</button>' +
          '<button class="glass-btn danger" onclick="ScienceApp.delArticle(\'' + a.id + '\')"><i class="fas fa-trash-alt"></i> 删除</button>' +
        '</div>';
  ...
  /* ============================================================
     ...
     - 内置情绪种类标签（开心/难过/生气/害怕/厌恶/焦虑/羡慕/尴尬/无聊，对标《头脑特工队》），按情绪分类分配主题色
  ...
  window.MoodFlowApp = {
    ...
    getEmotions: function () {
      ...
      return MOOD_BUILTIN_EMOTIONS.concat(custom);
    },
    getIntents: function () {
      var custom = [];
      try { custom = Storage.get(MOOD_INTENT_KEY, []); } catch (e) {}
      if (!Array.isArray(custom)) custom = [];
      return MOOD_BUILTIN_INTENTS.concat(custom);
    },
    ...
    renderTags: function () {
      ...
      var sbItems = emotions.map(function (e) {
        ...
        return '<button class="mood-sb-item' + active + '" data-emo="' + e.id + '" style="--mood-color:' + color + '">'
          + '<span class="mood-sb-dot"></span>'
          + '<span class="mood-sb-name">' + Core.escapeHtml(e.name) + '</span>'
          + '</button>';
      }).join('');
      ...
      var intentTags = curIntents.map(function (it) {
        return '<span class="mood-tag mood-tag-intent" data-text="' + Core.escapeHtml(it.text) + '" style="--mood-color:' + curColor + ';--mood-light:' + curLight + '">' + Core.escapeHtml(it.text) + '</span>';
      }).join('');

      return '<div class="mood-flow-shell">'
        + '<div class="mood-sidebar">' + sbItems + '</div>'
        + '<div class="mood-main">'
        + '<div class="mood-main-title"><i class="fas fa-face-smile"></i> ' + Core.escapeHtml(curName) + ' · 含义标签</div>'
    ...
    bindAddForm: function () {
      ...
      // 左侧情绪分类栏切换
      var sbItems = document.querySelectorAll('#love-test-container .mood-sb-item');
