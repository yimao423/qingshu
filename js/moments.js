...
const MomentsApp = {
  ...
  /* 进入朋友圈页时低频自动发布对方动态：独立冷却 key（20 分钟）+ 45% 概率，
     先写时间戳再发布，避免重复进入页面反复触发。 */
  _maybeAutoPostByRole() {
    try {
      var now = Date.now();
      var last = Storage.get('momentRoleAutoPostTime', 0) || 0;
      if (now - last < 20 * 60 * 1000) return;
      if (Math.random() >= 0.45) return;
      Storage.set('momentRoleAutoPostTime', now);
      if (this._getInteractRoles && this._getInteractRoles().length) {
        this.autoPostByRole();
      }
    } catch (e) {
      console.error('[moments] 自动发布对方动态失败', e);
    }
  },
