...
const Navigation = {
  ...
  _navigateTo(page, reverseAnim = '') {
    ...
    // 页面切换完成后重新采样底栏真实背景色，使 .bottom-nav 在任意页面/主题下与所处背景无肉眼分界
    if (window.ThemeManager && typeof ThemeManager.refreshNavBg === 'function') {
      try { ThemeManager.refreshNavBg(); } catch (e) {}
    }
  },
  ...
  _updateNav(page) {
    const tabMap = {
      ...
      'transmit': 'chat',
      'mt-echo': 'discover',
      'mt-echo-session-list': 'discover',
      'mt-echo-session': 'discover',
      'settings': 'settings',
  ...
  _renderPage(page) {
    switch (page) {
      ...
      case 'his-favorites':
        ...
        break;
      case 'his-favorites-group':
        window.renderHisFavoritesGroup && window.renderHisFavoritesGroup();
        break;
      case 'moments':
      ...
      case 'whereabout-reports':
        ...
        break;
      case 'transmit':
        window.renderTransmit && window.renderTransmit();
        break;
      case 'mt-echo':
        window.renderMtEcho && window.renderMtEcho();
        break;
      case 'mt-echo-session-list':
        window.renderMtEchoSessionList && window.renderMtEchoSessionList();
        break;
      case 'mt-echo-session':
        window.renderMtEchoSession && window.renderMtEchoSession();
        break;
      case 'settings':
