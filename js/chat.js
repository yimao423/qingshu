...
function renderChatMessages(chatId) {
  ...
  messages.forEach(function(msg) {
    ...
    if (msgDate !== lastDate) {
      lastDate = msgDate;
      // 当天日期分隔线每天只固定显示一次：标记已存在则跳过（lastDate 照常更新，同日期后续消息不再插入）
      var dateDividerKey = 'chatDateDividerShown_' + msgDate;
      var dateDividerShown = (msgDate === todayDateStr && localStorage.getItem(dateDividerKey) === '1');
      if (!dateDividerShown) {
        html += '<div class="chat-date-divider">' + msgDate + '</div>';
        if (msgDate === todayDateStr) {
          try { localStorage.setItem(dateDividerKey, '1'); } catch (e) {}
        }
      }
    }
...
function appendMessage(chatId, msg) {
  ...
  if (msgDate !== Core.formatDate(prevTime)) {
    // 当天日期分隔线每天只固定显示一次：标记已存在则跳过；历史日期始终照常插入
    var dateDividerKey = 'chatDateDividerShown_' + msgDate;
    var todayDateStr = Core.formatDate(new Date());
    var dateDividerShown = (msgDate === todayDateStr && localStorage.getItem(dateDividerKey) === '1');
    if (!dateDividerShown) {
