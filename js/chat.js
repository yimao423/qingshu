...
// 对方主动拍一拍混入概率：普通自动回复链路中对方主动发送拍一拍的触发概率（25%）
// （区别于 PAT_AUTO_REPLY_MIX_PROBABILITY：那是「我发拍一拍后对方回拍」的概率，这里是「普通聊天中对方主动拍一拍」的概率）
var PAT_ACTIVE_MIX_PROBABILITY = 0.25;
...
function _cleanBreakChars(text) {
  /* 清理消息文本中会导致意外断行的零宽/控制字符：
     1) 删除：零宽空格(ZWSP)、零宽非连接符、方向标记、BOM、字词连接符、
        软连字符(SHY)、行分隔符(LS)、段落分隔符(PS)——这些字符提供
        浏览器不可见的断行点，是"偶尔突然回车分段"的主要元凶；
     2) 归一为普通空格：EN/EM/THIN/HAIR 等各类 Unicode 空格及中数字空格
        （有宽度但极窄、肉眼难辨，且同样是合法断行点）；
     3) 保留：零宽连接符(ZWJ)以兼容 emoji 组合显示、不换行空格(NBSP)、
        全角空格（可见且有明确语义）、变体选择符(VS15/VS16) */
  return String(text || '')
    .replace(/[\u00ad\u200b\u200c\u200e\u200f\u2028\u2029]/g, '')
    .replace(/[\u2000-\u200a\u205f]/g, ' ')
    .replace(/\ufeff/g, '')
...
/* 聊天正文气泡换行：每行最多 maxLen 个字符，超出自动插入 <br> 换行。
   - 先统一清理换行符与零宽/不可见字符（含 \n \r BOM 软连字符 方向标记等，
     这些字符提供浏览器不可见断行点，是"某一行半截就回车"的主要元凶），
     然后整段连续文本按 maxLen 字符硬切分 → 不保留原文本手动换行，
     彻底杜绝"（夫妻一体）不是吗"这类不足 30 字的半截短行；
   - 基于 Unicode code point 切分，不会切开 emoji 代理对；
   - 先按字符数断行、后统一 escapeHtml，计数以真实字符为准；
   - 行数只由"字符数"决定，与字号/页面缩放无关；
     页面缩放到任意倍率、全局字体怎么调，都不会触发重排/回车错乱。 */
function _wrapTextEvery(text, maxLen) {
  var limit = (typeof maxLen === 'number' && maxLen > 0) ? Math.floor(maxLen) : 30;
  ...
  for (var k = 0; k < raw.length; k++) {
    var piece = raw[k];
    var c0 = raw.charCodeAt(k);
    if (c0 >= 0xd800 && c0 <= 0xdbff && k + 1 < raw.length) {
      var c1 = raw.charCodeAt(k + 1);
      if (c1 >= 0xdc00 && c1 <= 0xdfff) {
        piece = raw.slice(k, k + 2);
        k++;
      }
    }
    line += piece;
    n++;
    if (n >= limit && (k + 1) < raw.length) {
      html += Core.escapeHtml(line) + '<br>';
      line = '';
      n = 0;
    }
  }
  html += Core.escapeHtml(line);
  return html;
}
...
function _buildSingleMessageHtml(msg, opts) {
  ...
  if (msg.isPat) {
    ...
    html += '<div class="message-pat">'
      ...
      + Core.escapeHtml(_cleanBreakChars(msg.text || ''))
      + '<span class="pat-icon">' + patIconHtml + '</span>'
      + '</div>';
    ...
    html += '<div class="message-black-notice">'
      + (bnTarget ? '<span class="black-notice-target">' + Core.escapeHtml(_cleanBreakChars(bnTarget)) + '</span>：' : '')
      + Core.escapeHtml(_cleanBreakChars(bnBody))
      + '</div>';
    ...
    html += '<div class="message-punish">'
      ...
      + (pnTarget ? '<span class="punish-target">' + Core.escapeHtml(_cleanBreakChars(pnTarget)) + '</span>：' : '')
      + Core.escapeHtml(_cleanBreakChars(pnBody))
      + '</div>';
  } else if (msg.isCall) {
    ...
    html += '<div class="message-recall message-call' + callCls + '">' + Core.escapeHtml(_cleanBreakChars(msg.text || '')) + '</div>';
    ...
    html += '<div class="message-recall recall-near">' + Core.escapeHtml(_cleanBreakChars(recallText)) + '</div>';
    ...
    html += '<div class="mail-announce">'
      ...
      + '<span class="mail-announce-text">' + Core.escapeHtml(_cleanBreakChars(mnText)) + '</span>'
      + '</div></div>';
  } else {
...
function _buildNormalMessageHtml(msg, isSelf, selfAvatarHtml, otherAvatarHtml, suffixHtml, senderName, senderStatusHtml, rowGroupCls) {
  var html = '';
  // 传统微信/QQ 气泡：不再注入 BubbleMaker 的 ears/装饰/扩展类，统一走 CSS 默认气泡样式
  var senderHtml = '';
  if (senderName) {
  ...
  if (msg.msgType === 'sticker') {
    ...
    html += '<div class="message-row ' + (isSelf ? 'self' : 'other' + rowGroupCls) + '" data-msg-id="' + msg.id + '">'
          ...
          + '<div class="message-body">'
          + senderHtml
          + '<div class="message-bubble">' + quoteHtml + '<div class="bubble-text">' + _wrapTextEvery(msg.text, 30) + '</div>' + '</div>'
          + suffixHtml
          + '<div class="message-meta">' + _buildReadStatusHtml(msg) + '<div class="message-time">' + Core.formatTime(msg.time) + '</div></div>'
          + '</div>'
          + '</div>';
...
function sendMessage() {
  ...
  var text = _cleanBreakChars(input.value.trim());
  input.value = '';
  onChatInputChange();
...
// 概率取 Settings.getVoiceMixProb()（默认 20%，设置页可调）。
function _maybeSendVoiceAutoReply(chatId) {
  var mixProb = Storage.getVoiceMixProb();
...
function _sendVoiceFallbackTextReply(chatId) {
  ...
  var text = mainCards.length ? mainCards[Math.floor(Math.random() * mainCards.length)].text : '嗯嗯';
  var msgs = Storage.getMessages(chatId);
...
function _sendStickerAutoReply(chatId) {
  StickerDB.getRandom().then(function(sticker) {
    if (sticker && sticker.data) {
      ...
      // 表情包库为空时降级为普通回复
      var msgs = Storage.getMessages(chatId);
      var newMsg = { id: Date.now(), type: 'other', text: '嗯嗯', time: Date.now(), msgType: 'text' };
      msgs.push(newMsg);
      Storage.setMessages(chatId, msgs);
