...
function renderPaceSettings() {
  ...
  setToggle('cs-doodle-mixing', Storage.getDoodleMixing());
  ...
  var pfProb = Storage.getPartnerForwardProb();
  _paceSetInput('cs-partner-forward-prob', Math.round((pfProb == null ? 0.05 : pfProb) * 100));
}
...
// 对方转发消息概率（百分比 0-100，保存为 0~1 概率）
function chatSetPartnerForwardProb() {
  var el = document.getElementById('cs-partner-forward-prob-num');
  if (!el) return;
  var val = parseFloat(el.value);
  var current = Storage.getPartnerForwardProb();
  if (isNaN(val) || val < 0 || val > 100) {
    _paceSetInput('cs-partner-forward-prob', Math.round((current == null ? 0.05 : current) * 100));
    Core.toast('请输入 0-100 之间的数值');
    return;
  }
  Storage.setPartnerForwardProb(val / 100);
  Core.toast('对方转发消息概率已设为 ' + val + '%');
}
...
/* 通用文件字节读取（Promise<Uint8Array>）：
   优先 Blob.arrayBuffer()（Promise 化，对 file:// 与移动端浏览器更稳），失败回退
   FileReader.readAsArrayBuffer；两层都暴露 onerror / onabort 并转成明确错误，
   杜绝「点击确认文件后无任何反应」的静默失败（夸克/via/系统默认/Chrome 等移动端
   浏览器对 FileReader 大文件读取不稳定，且原实现无 onerror 回调、无 try/catch）。 */
function _readFileAsBytes(file) {
  return new Promise(function(resolve, reject) {
    if (!file) { reject(new Error('未选择文件')); return; }
    function _viaReader() {
      var reader = new FileReader();
      reader.onerror = function() { reject(new Error('读取文件失败（FileReader）')); };
      reader.onabort = function() { reject(new Error('读取文件已取消')); };
      reader.onload = function(ev) {
        var r = ev.target.result;
        if (r && r.byteLength !== undefined) { resolve(new Uint8Array(r)); }
        else if (r && typeof r === 'string') { resolve(new Uint8Array(new TextEncoder().encode(r))); }
        else { reject(new Error('无法读取文件内容')); }
      };
      try { reader.readAsArrayBuffer(file); }
      catch (eRead) { reject(eRead); }
    }
    if (file.arrayBuffer) {
      try {
        file.arrayBuffer().then(function(buf) { resolve(new Uint8Array(buf)); }).catch(function() { _viaReader(); });
        return;
...
function importChats() {
  ...
  input.onchange = function(e) {
    ...
    if (!file) return;
    var reader = new FileReader();
    reader.onerror = function() { Core.toast('读取文件失败，请重试'); };
    reader.onabort = function() { Core.toast('读取已取消'); };
    reader.onload = function(ev) {
      var data;
...
function importFullBackup() {
  ...
  input.onchange = function(e) {
    var file = e.target.files && e.target.files[0];
    if (!file) return;
    // 统一走 _readFileAsBytes：arrayBuffer() 优先，失败自动回退 FileReader，
    // 保证任何读取失败都会给出明确 toast，杜绝移动端「点击确认文件后无反应」。
    try {
      _readFileAsBytes(file).then(function(bytes) {
      ...
      Core.toast('读取文件失败：' + (errRead && errRead.message ? errRead.message : errRead));
    });
    } catch (err2) {
      Core.toast('导入出错：' + (err2 && err2.message ? err2.message : err2));
    }
  };
  input.value = '';
  input.click();
