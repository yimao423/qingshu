...
// 语音字卡混入概率滑条：初始化 + 拖动更新（5%~100%）
function syncVoiceMixProbUI() {
  var slider = document.getElementById('cs-voice-mix-prob');
  var valEl = document.getElementById('cs-voice-mix-prob-val');
  if (!slider || !valEl) return;
  var pct = Math.round(Storage.getVoiceMixProb() * 100);
  slider.value = pct;
  valEl.textContent = pct + '%';
}

function onVoiceMixProbInput(el) {
  var pct = parseInt(el.value, 10) || 20;
  Storage.setVoiceMixProb(pct / 100);
  var valEl = document.getElementById('cs-voice-mix-prob-val');
...
function updateRangeGradient(el) {
  ...
  var primary = style.getPropertyValue('--primary').trim() || '#4C9AFF';
  el.style.backgroundImage =
    'linear-gradient(to right, ' + primary + ' 0%, ' + primary + ' ' + pct + '%, ' +
    'rgba(255,255,255,0.2) ' + pct + '%, rgba(255,255,255,0.2) 100%)';
}
...
function exportChats() {
  ...
  // 导出瘦身（对标 ZY3）：消息中大图/大表情的 base64 抽取为轻量引用 + media 数据收集，
  // 导出 JSON 中不再携带大量内联 base64（旧消息内联大图同样被抽取）
  const media = {};
  ChatMedia.prepareExportMessages(allMessages, media, [0]).then(function() {
    const data = { chats, messages: allMessages, media, exportedAt: new Date().toISOString() };
...
/* 全量备份导出：聊天记录（单聊+群聊）+ 字卡 + 表情（含分类）+ 设置等打包下载。
   体积优化（对标 ZY3）：
   - 有 JSZip 时导出为 ZIP：backup.json 只留结构 + mediaIndex 引用，媒体 base64 转二进制
     写入 ZIP media/{key}，DEFLATE level 6 压缩，体积最小；
   - 无 JSZip 时用 CompressionStream('deflate') 兜底压缩单文件，并下调抽取阈值进一步瘦身；
   - 表情包（StickerDB）与 stickerCategories 分类一并导出，导入时写回。 */
function exportFullBackup() {
  try {
    ...
    // 抽取阈值：ZIP 模式媒体二进制化后已足够小，用默认阈值；兜底单文件模式下调阈值进一步瘦身
    var hasZip = typeof window.JSZip !== 'undefined';
    var threshold = hasZip ? ChatMedia.THRESHOLD : ChatMedia.EXPORT_THRESHOLD;
    var media = {};
    var seqBox = [0];
    var job = Promise.resolve();
    // 表情包（StickerDB）：深拷贝副本引用化（含 GIF 仅搬运不压缩），真实库不动
    var stickers = null;
    if (window.Storage && Storage.getStickersAsync) {
      job = job.then(function() {
        return Storage.getStickersAsync().then(function(list) {
          stickers = ChatMedia.collectStickersForExport(list, media, seqBox, threshold);
          return null;
        }).catch(function() { return null; });
      });
    }
    // 表情分类（StickerDB stickerCategories）
    var stickerCategories = [];
    if (window.Storage && Storage.getStickerCategoriesAsync) {
      job = job.then(function() {
        return Storage.getStickerCategoriesAsync().then(function(cats) {
          stickerCategories = cats || [];
          return null;
        }).catch(function() { return null; });
      });
    }
    job.then(function() {
      return ChatMedia.prepareExportMessages(messages, media, seqBox, threshold).then(function() {
        var data = {
          app: '拾心界',
          version: 3,
        ...
        if (hasZip) return buildZipBackup(data, media);
        return buildCompressedBackup(data, media);
      });
    }).then(function(res) {
...
/* ZIP 打包：media base64 → 二进制写入 ZIP media/{key}，backup.json 只留 mediaIndex 引用 + 结构数据 */
function buildZipBackup(data, media) {
  var zip = new JSZip();
  var mediaIndex = {};
  Object.keys(media).forEach(function(k) {
    try {
      var bin = ChatMedia.dataUrlToBinary(media[k]);
      zip.file('media/' + k, bin.bytes);
      mediaIndex[k] = { mime: bin.mime, size: bin.bytes.length };
    } catch (e) {
      mediaIndex[k] = { inline: true, data: media[k] }; // 个别异常 base64 内联兜底，导入仍可恢复
    }
  });
  ...
  return zip.generateAsync({
    ...
    return { blob: blob, name: '拾心界_全量备份_' + Core.formatDate(new Date()) + '.zip' };
  });
}

/* 兜底：无 JSZip 时用 CompressionStream('deflate') 压缩单 JSON（媒体内联 mediaStore），
   环境不支持 CompressionStream 时降级为原始 JSON（与旧版一致） */
function buildCompressedBackup(data, media) {
  data.media = media;
  ...
  if (window.CompressionStream) {
    try {
      ...
      return new Response(stream).blob().then(function(blob) {
        return { blob: blob, name: '拾心界_全量备份_' + Core.formatDate(new Date()) + '.backup' };
      });
    } catch (e) {}
  }
  var blob = new Blob([json], { type: 'application/json' });
  return Promise.resolve({ blob: blob, name: '拾心界_全量备份_' + Core.formatDate(new Date()) + '.json' });
}

/* 全量备份导入/恢复：支持 3 种备份文件——
   ① ZIP（v3，JSZip 解包 backup.json + media/* 二进制）；
   ② deflate 压缩单文件（v3 兜底，CompressionStream 解压，media 内联）；
   ③ 旧版单 JSON（v1/v2，直接解析，media 内联）。
  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(ev) {
      ...
      if (bytes.length > 4 && bytes[0] === 0x50 && bytes[1] === 0x4B && bytes[2] === 0x03 && bytes[3] === 0x04) {
        ...
        parsePromise = Promise.resolve().then(function() {             // 旧版单 JSON (v1/v2)
