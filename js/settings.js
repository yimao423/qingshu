...
function exportChats() {
  ...
  const allMessages = {};
  chats.forEach(c => {
    allMessages[c.id] = Storage.getMessages(c.id);
  });
  
  const data = { chats, messages: allMessages, exportedAt: new Date().toISOString() };
...
function exportFullBackup() {
  try {
    ...
    var groups = Storage.getGroupChats() || [];
    var messages = {};
    chats.forEach(function(c) { messages[c.id] = Storage.getMessages(c.id); });
    groups.forEach(function(g) { messages[g.id] = Storage.getMessages(g.id); });
  ...
  Core.dangerConfirm('重置全部数据', '【高危操作】将清空 localStorage 与 IndexedDB 中的全部数据（聊天记录、字卡、设置、表情包等），且无法恢复。确定继续？', function() {
    Core.dangerConfirm('再次确认', '此操作不可撤销！请再次确认要重置全部数据吗？', function() {
      try {
        ...
        // 清空 IndexedDB（先走各 DB 的 clear 方法，再尝试 deleteDatabase 兜底）
        var tasks = [];
        ...
        try { indexedDB.deleteDatabase('mirror_call_bg_db'); } catch (e) {}
