  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(ev) {
      ...
      Core.dangerConfirm('导入聊天记录', '将从备份文件合并约 ' + total + ' 条消息到对应聊天（按消息 id 去重，不覆盖现有数据）。确定导入？', function() {
        try {
          // 0) 恢复备份中的大图 media 到 IndexedDB（消息中的轻量引用保持不变，渲染时异步还原）
          ChatMedia.restoreMedia(data.media);
...
/* 全量备份导出：聊天记录（单聊+群聊）+ 字卡 + 表情 + 设置等打包为一个 JSON 文件下载。
   体积优化（对标 ZY3）：消息大图/大表情抽取为轻量引用 + media 收集；表情库大图同样引用化，
   导出 JSON 只保留轻量引用与一份 media 数据，避免大量原始大 base64 重复内联。 */
function exportFullBackup() {
  try {
    ...
    // 消息瘦身 + 表情库瘦身（均在副本上操作，不改动真实存储）
    var media = {};
    var seqBox = [0];
    var stickerJobs = [];
    if (window.Storage && Storage.getStickersAsync) {
      stickerJobs.push(Storage.getStickersAsync().then(function(stickers) {
        // 深拷贝：导出瘦身只作用于副本，避免把 StickerDB 真实数据原地改成引用
        var copy = (stickers || []).map(function(s) { return JSON.parse(JSON.stringify(s)); });
        copy.forEach(function(s) {
          if (!s || !s.data || typeof s.data !== 'string' || s.data.indexOf('data:') !== 0) return;
          // GIF 表情保持原样导出（动图不可压缩），仅大体积静态图引用化
          if (s.data.length > ChatMedia.THRESHOLD && s.data.indexOf('data:image/gif') !== 0) {
            var key = 'lib_' + (s.id || ('s' + seqBox[0]++));
            media[key] = s.data;
            s.data = ChatMedia.makeRef(key);
          }
        });
        return copy;
      }).catch(function() { return null; }));
    }
    Promise.all(stickerJobs).then(function(stickerResults) {
      var stickers = stickerResults[0] || null;
      return ChatMedia.prepareExportMessages(messages, media, seqBox).then(function() {
        var data = {
          app: '拾心界',
          version: 2,
...
/* 全量备份导入/恢复：从备份 JSON 恢复全部数据（危险操作，二次确认后覆盖当前数据并刷新） */
function importFullBackup() {
  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(ev) {
      ...
      Core.dangerConfirm('恢复全量备份', '将用备份文件覆盖当前所有数据（聊天记录、字卡、设置等），当前数据会被替换且不可恢复。确定恢复？', function() {
        try {
          // 1) 聊天列表与群聊
          ...
          // 2.1) 恢复备份中的大图 media 到 IndexedDB（消息保持轻量引用，渲染时异步还原）
          ChatMedia.restoreMedia(data.media);
          // 2.2) 表情库：v2 备份中引用化的表情恢复为大图，GIF 表情保持原样
          if (data.stickers && Array.isArray(data.stickers) && window.Storage && Storage.setStickersAsync) {
            Promise.all(data.stickers.map(function(s) { return ChatMedia.resolveStickerForImport(s); })).then(function(resolved) {
              Storage.setStickersAsync(resolved);
            });
          }
          // 3) 字卡 / 表情 / 颜文字
          ...
          Core.toast('恢复失败：' + (err2 && err2.message ? err2.message : err2));
        }
      });
    };
    reader.readAsText(file);
