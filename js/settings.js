...
function renderAppearance() {
  ...
  if (fontSizeSlider) {
    ...
    if (fontSizeValue) fontSizeValue.textContent = Math.round(Storage.getFontSize() * 100) + '%';
    updateRangeGradient(fontSizeSlider);
  }
}

function changeFontSize(val) {
  Storage.setFontSize(parseFloat(val));
...
function exportChats() {
  ...
  a.href = url;
  a.download = '疯狗龙的情书_聊天记录_' + Core.formatDate(new Date()) + '.json';
  a.click();
