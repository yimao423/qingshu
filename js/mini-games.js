...
(function () {
  ...
  var SheepGame = {
    ...
    _buildCards: function () {
      ...
      this.cards = [];
      var id = 0;
      // 每层牌数：3*4 + 2*3 + 1*2 = 12+6+2 = 20，调整为 18（3倍数）可消
      var plan = [
      ...
      // 保证 3 的倍数
      if (total % 3 !== 0) { plan[1].count += (3 - total % 3); }
      var emojiPool = [];
      var per = Math.floor(SHEEP_EMOJIS.length * 3 / 3); // 种类数
      var needTypes = Math.ceil((plan[0].count + plan[1].count) / 3);
    ...
    _render: function () {
      ...
      var html =
        '<div class="game-topbar glass-card">' +
          '<div class="game-topbar-info"><span class="chip">剩余 ' + this._leftCount() + ' 张</span><span class="chip">求助 <b id="sheep-help-left">' + (this.maxHelp - this.helpCount) + '</b>/' + this.maxHelp + '</span></div>' +
  ...
  var GooseGame = {
    ...
    _render: function () {
      ...
      var html =
        '<div class="game-topbar glass-card">' +
          '<div class="game-topbar-info"><span class="chip">分数 <b id="goose-score">' + this.score + '</b>/' + this.target + '</span><span class="chip">步数 <b id="goose-moves">' + this.moves + '</b></span><span class="chip">求助 <b id="goose-help-left">' + (this.maxHelp - this.helpCount) + '</b>/' + this.maxHelp + '</span></div>' +
          '<div class="game-topbar-actions">' +
        ...
        '<div class="goose-tip">点击选中一块，再点相邻的交换；连成 3 个及以上即可消除，步数内达到 ' + this.target + ' 分获胜</div>' +
        '<div class="game-actions">' +
    ...
    _checkState: function () {
      ...
      if (this.score >= this.target) {
        this.running = false;
        showOverlay('🎉', '通关啦！', getRewardQuote(), '再来一局', function () { GooseGame.restart(); });
        return;
      }
      if (this.moves <= 0) {
        // 检查是否还有可消
        if (this._findMatches().length) {
          // 还有连锁但步数用完 → 失败
          this.running = false;
          showOverlay('😢', '步数用完啦', '还差一点，让 TA 帮帮忙，或者重开一局吧～', '再试一次', function () { GooseGame.restart(); });
        }
      }
    },
    ...
    _updateTopbar: function () {
      ...
      var m = this.container.querySelector('#goose-moves');
  ...
  window.renderMiniGames = function () {
    ...
    if (!container) return;
    container.innerHTML =
      '<p class="games-subtitle">和 TA 一起玩的五款小游戏，卡壳时点「求助 TA」让 TA 来帮你～</p>' +
      '<div class="games-list">' +
        ...
        '<div class="game-entry glass-card" onclick="Navigation.navigateTo(\'game-goose\')">' +
          ...
          '<div class="game-entry-info"><h3>消消乐</h3><p>7×7 网格交换三消，连成 3 个消除得分，步数内冲上目标分！</p><span class="game-badge">交换三消</span></div>' +
          '<i class="fas fa-chevron-right game-entry-arrow"></i>' +
        '</div>' +
  ...
  var MemoryGame = {
    ...
    _renderBoard: function () {
      ...
      this.cards.forEach(function (card, idx) {
        var cell = document.createElement('div');
        cell.className = 'memory-card' + (card.matched ? ' matched' : (card.flipped ? ' flipped' : ''));
        cell.dataset.idx = idx;
        cell.innerHTML =
