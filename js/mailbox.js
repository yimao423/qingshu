...
const MailboxApp = {
  ...
  /* ---------- 角色工具（来信/写信按角色归属） ---------- */
  // 按角色 id 查找角色资料；无匹配返回 null
  _partnerById(partnerId) {
  ...
  // 解析一封信的归属角色：来信优先用 fromId，寄出信/回信用 toId；
  // 历史数据无角色字段时回退到第一个角色（默认 TA），保证旧信件也能归组展示
  _resolveLetterPartner(l) {
    var p = null;
    if (l) {
      if (l.fromId) p = this._partnerById(l.fromId);
      if (!p && l.toId) p = this._partnerById(l.toId);
    }
    if (!p) {
      var partners = Storage.getPartnerProfiles() || [];
      p = partners.length ? partners[0] : null;
    }
    return p;
  },

  // 角色显示名：优先使用角色昵称，兜底 TA
  _partnerDisplayName(p) {
  ...
  // 生成一封对方主动来信（内容 = 主字卡；发送方随机绑定一个角色，多角色时分组成员更清晰）
  _spawnIncoming(data, now, settings) {
    ...
    // 角色绑定：按轮询游标循环分配来信角色，确保每个角色都能收到来信（无角色时保持旧行为，不附加字段）
    var partners = Storage.getPartnerProfiles() || [];
    if (partners.length) {
      var idx = Number(data.nextIncomingRoleIdx) || 0;
      var fromPartner = partners[idx % partners.length];
      letter.fromId = fromPartner.id;
      letter.fromName = fromPartner.nickname || fromPartner.name || 'TA';
      // 游标前进到下一个角色（越界回绕），使来信在角色间轮转
      data.nextIncomingRoleIdx = (idx + 1) % partners.length;
    }
    data.letters.unshift(letter);
  },

  // 对方自动回信（对我方寄出的信，内容 = 主字卡；回信归属 = 该信收信角色）
  _autoReply(data, letter, now, settings) {
    ...
    if (letter.toId) {
      letter.fromId = letter.toId;
      letter.fromName = letter.toName || 'TA';
    }
  },
  ...
  switchMailboxTab(tab) {
    ...
    // 进入「时空来信」时回到第一级角色列表
    if (this._mailboxTab === 'inbox') this._mailboxInboxRoleId = null;
    this.renderMailbox();
  },
  ...
  // 「时空来信」Tab：两级导航——第一级展示角色列表，点击角色进入第二级查看该角色来信
  _buildInboxTabHtml(data) {
    ...
    // 无可用角色时退回平铺列表（兼容单角色/无角色旧数据）
    if (!partners.length) {
      inLetters.forEach(function (l) { listHtml += self._buildIncomingItem(l); });
      outReplied.forEach(function (l) { listHtml += self._buildReplyItem(l); });
      if (!listHtml) {
    ...
    // 按角色收集信件：历史无角色字段的信归第一个角色，保证旧信件可见
    var lettersByRole = {};
    partners.forEach(function (p) { lettersByRole['p_' + p.id] = []; });
    function roleKey(letter) {
      var partner = self._resolveLetterPartner(letter);
      return partner && partner.id ? 'p_' + partner.id : 'p_' + partners[0].id;
    }
    inLetters.forEach(function (l) { lettersByRole[roleKey(l)].push(self._buildIncomingItem(l)); });
    outReplied.forEach(function (l) { lettersByRole[roleKey(l)].push(self._buildReplyItem(l)); });

    // 第一级：角色列表
    var currentRoleId = this._mailboxInboxRoleId || null;
    if (!currentRoleId) {
      var roleItems = '';
      partners.forEach(function (p) {
        var cnt = (lettersByRole['p_' + p.id] || []).length;
        roleItems += '<div class="mailbox-role-item" onclick="MailboxApp.openInboxRole(\'' + Core.escapeHtml(p.id) + '\')">'
          + self._buildRoleAvatar(p)
          + '<div class="mailbox-role-info">'
          + '<div class="mailbox-role-name">' + Core.escapeHtml(self._partnerDisplayName(p)) + '</div>'
          + '<div class="mailbox-role-sub">' + (cnt ? '来信 ' + cnt + ' 封' : '还没有来信') + '</div>'
          + '</div>'
          + (cnt ? '<span class="mailbox-role-count">' + cnt + '</span>' : '')
          + '<i class="fas fa-chevron-right mailbox-role-arrow"></i>'
          + '</div>';
      });
      if (!roleItems) {
        roleItems = '<div class="mailbox-empty"><i class="fas fa-envelope-open-text"></i>还没有设置角色，去设置页添加吧</div>';
      }
      return '<div class="mailbox-section">'
        + '<div class="mailbox-section-title"><span class="sec-icon"><i class="fas fa-envelope-open-text"></i></span>时空来信<span class="sec-count">' + (inLetters.length + outReplied.length) + '</span></div>'
        + roleItems
        + '</div>';
    }

    // 第二级：当前角色的来信列表（含返回角色列表入口）
    var currentPartner = null;
    partners.forEach(function (p) { if (String(p.id) === String(currentRoleId)) currentPartner = p; });
    if (!currentPartner) currentPartner = partners[0];
    var items = lettersByRole['p_' + currentPartner.id] || [];
    var body = items.join('');
    if (!body) {
      body = '<div class="mailbox-empty"><i class="fas fa-envelope-open-text"></i>TA 还没有来信，去写一封信吧</div>';
    }
    return '<div class="mailbox-section">'
      + '<div class="mailbox-role-header">'
      + '<button class="mailbox-back-btn" onclick="MailboxApp.openInboxRole(null)" title="返回角色列表"><i class="fas fa-arrow-left"></i></button>'
      + self._buildRoleAvatar(currentPartner)
      + '<span class="mailbox-role-header-name">' + Core.escapeHtml(self._partnerDisplayName(currentPartner)) + ' 的来信</span>'
  ...
  // 进入/返回角色来信：roleId 为 null 时回到角色列表（第一级）
  openInboxRole(roleId) {
    this._mailboxInboxRoleId = roleId || null;
    this.renderMailbox();
  },

  // 角色头像：优先 avatarImage 图片，其次 avatar 文本首字，兜底默认信封图标
  _buildRoleAvatar(p) {
    if (!p) return '<span class="mailbox-role-avatar is-default"><i class="fas fa-envelope"></i></span>';
    var colorStyle = p.avatarColor ? ' style="background:' + Core.escapeHtml(p.avatarColor) + '"' : '';
    if (p.avatarImage) {
      return '<span class="mailbox-role-avatar"' + colorStyle + '><img src="' + Core.escapeHtml(p.avatarImage) + '" alt="" onerror="this.style.display=\'none\'"></span>';
    }
    var text = p.avatar ? String(p.avatar).charAt(0) : '信';
    return '<span class="mailbox-role-avatar"' + colorStyle + '>' + Core.escapeHtml(text) + '</span>';
  },

  // 我方寄出信列表项（点击查看详情）
  _buildOutLetterItem(l) {
    var toName = l.toName || this._partnerDisplayName(this._resolveLetterPartner(l));
    var footerHtml = '';
    ...
    return '<div class="letter-note is-out is-clickable" onclick="openMailboxDetail(\'' + l.id + '\')">'
      ...
      + '<span class="letter-role-tag"><i class="fas fa-paper-plane"></i> 寄给 ' + Core.escapeHtml(toName) + '</span>'
  ...
  _buildIncomingItem(l) {
    var fromName = l.fromName || this._partnerDisplayName(this._resolveLetterPartner(l));
    var footerHtml = '';
    if (l.status === 'replied' && l.replyContent) {
      footerHtml = '<div class="letter-note-footer">'
    ...
    return '<div class="letter-note is-incoming is-clickable" onclick="openMailboxDetail(\'' + l.id + '\')">'
      + '<div class="letter-note-header">'
      + '<span class="letter-role-tag"><i class="fas fa-user"></i> ' + Core.escapeHtml(fromName) + ' 的来信</span>'
      + '<span class="letter-time">' + Core.formatTime(l.sendTime) + '</span>'
      + '</div>'
      + '<div class="letter-note-content letter-note-clamp">' + Core.escapeHtml(l.content) + '</div>'
      + footerHtml
  ...
  _buildReplyItem(l) {
    ...
    return '<div class="letter-note is-reply is-clickable" onclick="openMailboxDetail(\'' + l.id + '\')">'
      + '<div class="letter-note-header">'
      + '<span class="letter-reply-badge"><i class="fas fa-reply"></i> ' + Core.escapeHtml(toName) + ' 的回信</span>'
  ...
  _renderWritePage() {
    ...
    // 多角色时提供收信人选择：写信对象决定该信归属角色，回信也将来自该角色
    var toSelectorHtml = '';
    if (partners.length > 1) {
      toSelectorHtml = '<div class="letter-to-row">'
        + '<span class="letter-to-label"><i class="fas fa-user-tag"></i> 收信人</span>'
        + '<select id="letter-to-select" class="letter-to-select" onchange="MailboxApp.onWriteTargetChange()">';
      partners.forEach(function (p, idx) {
        toSelectorHtml += '<option value="' + Core.escapeHtml(p.id) + '"' + (idx === 0 ? ' selected' : '') + '>'
          + Core.escapeHtml(p.nickname || p.name || 'TA') + '</option>';
      });
      toSelectorHtml += '</select></div>';
    }
    editor.innerHTML = toSelectorHtml + '<div class="letter-editor">'
  ...
  // 收信人切换：同步更新写信占位提示中的称呼
  onWriteTargetChange() {
    var sel = document.getElementById('letter-to-select');
    var ta = document.getElementById('letter-content');
    if (!sel || !ta) return;
    var partners = Storage.getPartnerProfiles() || [];
    var name = 'TA';
    for (var i = 0; i < partners.length; i++) {
      if (String(partners[i].id) === sel.value) { name = partners[i].nickname || partners[i].name || 'TA'; break; }
    }
    ta.placeholder = '写给亲爱的' + name + '：\n\n写下你想说的话...';
  },
  ...
  sendLetter() {
    ...
    // 收信角色：写信页存在收信人选择时取其选择，否则默认第一个角色
    var partners = Storage.getPartnerProfiles() || [];
    var toPartner = null;
    var toSel = document.getElementById('letter-to-select');
    if (toSel && toSel.value) {
      for (var i = 0; i < partners.length; i++) {
        if (String(partners[i].id) === toSel.value) { toPartner = partners[i]; break; }
      }
    }
    if (!toPartner) toPartner = partners.length ? partners[0] : null;
    ...
    if (toPartner) {
      letter.toId = toPartner.id;
      letter.toName = toPartner.nickname || toPartner.name || 'TA';
    }
  ...
  // 单人聊天公告通知：向该信收信角色（letter.toId，默认第一个角色）的聊天写入 isMailNotice 系统消息
  _pushChatAnnounce(letter) {
    var partners = Storage.getPartnerProfiles() || [];
    if (!partners.length) return;
    var targetId = letter.toId || partners[0].id;
    var chatId = 'partner_' + targetId;
  ...
  openReplyLetter(letterId) {
    ...
    if (!container) return;
    var fromName = letter.fromName || this._partnerDisplayName(this._resolveLetterPartner(letter));
    container.innerHTML = '<div class="reply-editor">'
      + '<div class="reply-target-note">'
      + '<div class="reply-target-title"><i class="fas fa-envelope"></i> 回复 ' + Core.escapeHtml(fromName) + ' 的来信</div>'
